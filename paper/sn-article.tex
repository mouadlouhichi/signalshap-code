...
We ship that experiment (\texttt{scripts/run\_protocol\_sensitivity.py}) rather
than leave it as a suggestion: it fits the head in the validation state,
history $=$ train, and scores the coalitions in the test state, history $=$
train $+$ $i_u^{\mathrm{val}}$, so the refreshed sources see the event and the
consumed item is masked. On a synthetic pilot the refreshed protocol scores
\emph{higher} rather than lower ($v(\G)$ $0.147 \to 0.165$) and moves two
signs, which is the concrete reason the lower-bound claim had to go. We report
the corpus results with the next full regeneration.
...
concerns. The same script refits with those users dropped, so the sensitivity
of the attributions to the retention policy is measured rather than asserted;
on the synthetic pilot the two policies agree to Kendall $\tau = 1.00$ with
$\max_g|\Delta\varphi_g| = 5.0\times10^{-4}$.
...
validation candidate recall bounds the fit in the same way. The released
pipeline records it, together with the number of users whose validation
positive is unretrieved, in every run artefact; the runs behind this table
predate that instrumentation, so we state the policy here and report the
measurement with the next full regeneration rather than quote a number we have
not yet computed. Users whose validation item is unretrieved are kept, contributing to
...
training fold, so the validation interaction is neither added to the history nor
masked before test scoring. The direction of the resulting error is known and
conservative, since an already-consumed but still rankable item can only
displace the test item downward, but the magnitude is unmeasured and the
sequential and recency sources are the ones most plausibly penalised.
