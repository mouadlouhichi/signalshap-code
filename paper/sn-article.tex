...
the game. All three components are seed-independent, so $\Cu$ is
...
One disclosure. The symmetric key is the default in the released code, but the
attribution tables in Sections~\ref{sec:results} and~\ref{sec:estimands} were
produced under the legacy key, and we have regenerated only MovieLens-1M under
the new one: a full three-corpus, ten-seed regeneration costs roughly the
wall-clock of the original study, most of it Gowalla. We report the head-to-head
comparison above on the corpus that carries the main claims rather than assert
that the difference is negligible elsewhere, and the two artefacts are released
side by side so the comparison can be checked.
...
contribution of $g$ with a change of source state. It is not free, though, and
the cost falls in a known direction. Because $i_u^{\mathrm{val}}$ is already
consumed but still rankable, it can occupy a top-$K$ slot and push the test item
\emph{down}; it can never raise $\NDCG$. The protocol is therefore
\emph{conservative}: every reported $v(S)$, including $v(\G)$, is a lower bound
on what the same sources would score with a refreshed state, and no optimistic
leakage is possible. The penalty applies to all $32$ coalitions alike, which is
why we expect the attribution contrasts to be far less affected than the
absolute values, but we have not measured that and do not claim it. The
sequential and recency sources are the ones a refreshed state would most
plausibly favour, so this protocol is if anything unkind to them.
...
that vector is all zero and the user contributes negatives only. We report
validation candidate recall alongside test recall in
Table~\ref{tab:preconditions} so the size of this effect is visible rather than
implicit, and we retain the all-zero users: dropping them would fit the head on
a subpopulation selected for being easy to retrieve, which is a worse bias than
the one it removes. The Gram accumulation in Equation~(\ref{eq:head}) is
$|C_u|$-normalised, so such users contribute to $A$ but nothing to $c$, shrinking
the fitted weights slightly rather than distorting their direction.
...
resamples over. Single-run diagnostics that are illustrative rather than inferential (the
interaction indices, the estimand comparison, the per-corpus robustness sweeps)
are reported at seed $42$ and labelled as such.
...
pipeline now records it in every run artefact, and we report it in the final
version. Users whose validation item is unretrieved are kept, contributing to
...
($42$--$51$) and the error bars span $\pm1.96$ standard errors over those ten,
...
means the pair jointly contributes less than the sum of their separate contributions, the signature of redundancy, and a positive value indicates complementarity.
...
baseline no interval spans zero on any corpus. An interval for the mean is not
by itself a statement about individual seeds, so we also counted directly: all
fifteen source-by-corpus cells have the same sign on $10$ of $10$ seeds,
including the two near-zero cases ($\varphi_{ct}$ on MovieLens, mean
$-0.00018$, negative on $10/10$; $\varphi_{rec}$ on Amazon-VG, mean $-0.00049$,
negative on $10/10$) whose signs were not stable under the sampled baseline.
Intervals are $\bar{x} \pm t_9\,\mathrm{SE}$ over the ten seeds; an earlier
version used the normal quantile $1.96$, which is too narrow at $n = 10$, and
widening them changes no conclusion here. The $\LOOr$ and gap columns are
seed-42 point estimates. We have computed ten-seed intervals for both on
MovieLens-1M, where the material $cf$ case is
stable in every respect ($\LOOr = -0.00477$, negative on $10/10$ seeds; gap
$= +0.02033$, $[+0.01990, +0.02077]$, positive on $10/10$). The near-zero
sources are not stable: $\LOOr(ct)$ is positive on only $7/10$ seeds and
$\LOOr(rec)$ on $5/10$, so the two negligible flips on this corpus are seed
noise and we do not interpret them. Amazon-VG and Gowalla intervals for $\LOOr$
and for the gap are not yet computed, which is a real gap in the evidence for
the Gowalla $pop$ disagreement: that result currently rests on a seed-42 point
estimate plus the ten-seed interval on $\varphi_{pop}$ alone. The released
runner now records per-seed $\LOOr$ and gap on every corpus, so the next full
run closes it.}
\label{tab:loo}
\begin{tabular}{@{}llrrcrl@{}}
\toprule
Corpus & Source & $\LOOr$ & Shapley & 10-seed mean $[95\%$ CI$]$ & Gap & \\
       &        & (seed 42) & (seed 42) & & (seed 42) & \\
\midrule
MovieLens-1M & $cf$ & $-0.00519$ & $+0.01545$ & $+0.01574\ [+0.01542,+0.01607]$ & $+0.02064$ & \textbf{flip, material} \\
             & $ct$ & $+0.00044$ & $-0.00009$ & $-0.00018\ [-0.00024,-0.00011]$ & $-0.00053$ & flip, negligible \\
             & $pop$ & $+0.00065$ & $+0.00637$ & $+0.00646\ [+0.00638,+0.00653]$ & $+0.00572$ &  \\
             & $rec$ & $-0.00005$ & $+0.00026$ & $+0.00024\ [+0.00014,+0.00035]$ & $+0.00031$ & flip, negligible \\
             & $seq$ & $+0.01635$ & $+0.02937$ & $+0.02992\ [+0.02962,+0.03022]$ & $+0.01302$ &  \\
\midrule
Amazon-VG    & $cf$ & $+0.00845$ & $+0.01926$ & $+0.01979\ [+0.01937,+0.02022]$ & $+0.01081$ &  \\
             & $ct$ & $+0.00002$ & $+0.00187$ & $+0.00195\ [+0.00183,+0.00207]$ & $+0.00185$ &  \\
             & $pop$ & $+0.00308$ & $+0.00157$ & $+0.00178\ [+0.00165,+0.00191]$ & $-0.00152$ &  \\
             & $rec$ & $+0.00023$ & $-0.00047$ & $-0.00049\ [-0.00053,-0.00045]$ & $-0.00070$ & flip, negligible \\
             & $seq$ & $+0.00758$ & $+0.01909$ & $+0.01883\ [+0.01865,+0.01900]$ & $+0.01151$ &  \\
\midrule
Gowalla      & $cf$ & $+0.00729$ & $+0.00711$ & $+0.00642\ [+0.00630,+0.00653]$ & $-0.00018$ &  \\
             & $ct$ & $+0.00528$ & $+0.00674$ & $+0.00961\ [+0.00955,+0.00967]$ & $+0.00146$ &  \\
             & $pop$ & $-0.00151$ & $+0.00055$ & $+0.00092\ [+0.00085,+0.00100]$ & $+0.00206$ & \textbf{flip, material} \\
             & $rec$ & $+0.00001$ & $+0.00038$ & $+0.00035\ [+0.00030,+0.00039]$ & $+0.00037$ &  \\
             & $seq$ & $+0.00070$ & $+0.00213$ & $+0.00360\ [+0.00354,+0.00367]$ & $+0.00143$ &  \\
\botrule
\end{tabular}
\end{table}
...
On MovieLens it is $cf$: Shapley scores it $+0.01545$, the second most valuable
source in the system, where $\LOOr$ scores it $-0.00519$ and marks it as worth
removing. The gap is $0.02064$, or $40.2\%$ of $v(\G) = 0.05136$. On Gowalla it is $pop$, at
...
The magnitudes, however, are \emph{not} stable, and we report this against our
own convenience. A full ten-seed repetition at the smallest budget draws
$4\,652$ of $8\,865$ users and $59\,597$ of $82\,134$ items, which also raises
the frozen $N_{\max} = 11\,623$ from $14.2\%$ to $19.5\%$ of the catalogue.
Under that resampling the Gowalla attributions move by up to $36\%$ in relative
terms ($\varphi_{ct}$ from $0.00961$ to $0.01310$), while the source ordering,
every sign, and the top source are unchanged (Kendall $\tau = 1.00$). The same
repetition on MovieLens-1M and Amazon-VG, whose user caps are not budget-bound
and whose corpora are therefore byte-identical across the two runs, reproduces
to within $4.1\%$ and $0.2\%$ respectively, which is a genuine cross-run
reproduction of those two corpora. The conclusion we draw is narrow: on the
sparsest corpus the \emph{ordering} is robust to which half of the users is
sampled, the \emph{magnitudes} are not, and Gowalla point estimates should be
read accordingly.
...
\paragraph{The reported results predate the final candidate rule}
We state this as a limitation rather than an implementation footnote, because
it is the largest gap between the method of record and the evidence. The
source-symmetric truncation key of Section~\ref{sec:game} is the declared
method and the released default, but every table in
Sections~\ref{sec:results} and~\ref{sec:estimands} was produced under the
superseded order-dependent key, and only MovieLens has been recomputed under
both. On that corpus the two agree to $\max_g|\Delta\varphi_g| = 2.8\times
10^{-5}$ with identical ordering, signs and material flip, which is why we
consider the reported conclusions safe; it is not a substitute for regenerating
Amazon-VG and Gowalla, which costs roughly the wall-clock of the original study
and which we have not spent. A machine-readable map from every table and figure
to the artefact, platform, candidate rule and seeds that produced it ships with
the repository.
...
intervals at $n = 10$. The $\tau$ intervals are normal-approximation intervals
over the ten seed-level correlations, $\bar\tau \pm 1.96\,\mathrm{SE}$, and are
degenerate where every seed returns the same value; with $n = 10$ they should
...
Corpus & $\tau$ $\LOOr$ & $\tau$ Shapley
       ...
       & (95\% CI) & (95\% CI) & & \\
\midrule
MovieLens-1M & $0.96\ [0.91, 1.00]$ & $0.20\ [0.20, 0.20]$
             & $100\%\ [72, 100]$ & $0\%\ [0, 28]$ \\
Amazon-VG    & $0.94\ [0.88, 1.00]$ & $0.76\ [0.68, 0.84]$
             & $80\%\ [49, 94]$   & $50\%\ [24, 76]$ \\
...
more measured. $\LOOr$'s mean rank agreement is $0.96$, $0.94$ and $1.00$;
Shapley's is $0.20$, $0.76$ and $0.80$. $\LOOr$ is therefore \emph{not} perfect on every
...
and Gowalla survives the interval while the Amazon contrast, $[49, 94]$ against
$[24, 76]$, does not.
...
\tau_{\text{Shapley},s}$ is both the correct statistic and a tighter one. On
MovieLens-1M and Gowalla, Shapley's correlation is identical on all ten seeds,
so the paired difference is exactly recoverable: $\Delta\tau = +0.76$
$[+0.70, +0.82]$ and $\Delta\tau = +0.20$ with zero variance, positive on
$10/10$ seeds in both cases. Neither interval contains zero. On Amazon-VG both
correlations vary across seeds and the per-seed values were not retained, so we
cannot reconstruct the pairing after the fact and do not report it; the
released runner now records per-seed $\tau$ and the paired bootstrap on every
corpus. With ten seeds these intervals are indicative rather than calibrated.
...
Divide credit for current quality
  & Shapley, or a declared semivalue
...
\caption{Segment-level Shapley values, with segment sizes in the legend. The
four segments cover the $6\,035$ users the game evaluates, which is three fewer
than the $6\,038$ in Table~\ref{tab:preconditions}: three users retrieve an
empty candidate set and are therefore outside every $v(S)$, not excluded by the
segmentation. Overlapping markers within a source indicate little heterogeneity
to exploit.
