...
buys something unusual. Feature-level Shapley must approximate, because the coalition lattice over hundreds of features is intractable,
...
A source-symmetric key, for instance the mean reciprocal rank across sources, would remove it, and we would adopt one in a revision of the
design.

This choice is load-bearing. The obvious alternative, drawing one candidate pool from the full fused scorer, is \emph{invalid}, because it hands the grand
...
overfit the held-out interaction. That would inflate $v(\G)$ relative to small coalitions. This is the same pathology as a grand-coalition candidate pool, arriving
...
($\chi^2 = 14.24$, $p = 0.014$) and the mean ranks order the methods as one would expect (SignalShap-Fuse $1.33$, globally tuned head $1.67$, uniform $3.00$, LightGCN $4.00$, popularity $5.33$, SASRec $5.67$), but with three blocks the critical difference is $4.35$ and \emph{no pair is separable}.
...
That is inconsistent with our own stated policy. We keep the test only because its conclusion is negative, nothing separates, so it cannot manufacture a claim the gate was meant to prevent; a positive result here would have had to
...
\item \textbf{Author contributions.} \textbf{M.\ Louhichi:} conceptualisation,
      ...
      supervision, methodology, resources, writing (review and editing). \item \textbf{Use of AI tools.} Large language models were used for code
