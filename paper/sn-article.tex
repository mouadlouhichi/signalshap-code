...
rather than retirement guidance. Only one corpus clears our candidate-recall
gate; on the sparsest, reaching it would require a candidate pool so large that
the fixed-candidate design collapses into full-catalogue ranking. Code and
every backing artefact are released.}
...
\item \textbf{C3b.} Evidence that the credit SignalShap assigns is
      ...
      the claim is that the two rules diverge on some source in every system,
      not that ablation systematically undervalues a particular one
...
\item \textbf{C5.} Fusion weights derived from these attributions beat
      LightGCN, SASRec, uniform, and popularity baselines at $p<0.001$ ---
      though we note the neural baselines received smaller tuning budgets
      (Section~\ref{sec:threats}). Against a well-tuned global head there is
      \emph{no} advantage: the difference is $-0.00005$ on MovieLens and a
      pre-specified equivalence test concludes the two are equivalent within
      $0.002$ $\NDCG$. Attribution-derived fusion is therefore not a
      contribution of this paper, and we report the negative result rather than
      the favourable framing of an earlier draft.
...
natural comparators, and because redundancy between sources is central to our
setting we evaluate both rather than deferring them
...
seed-interval and equivalence analyses of Section~\ref{sec:segments} use ten
...
a source whose value is absorbed by a redundant partner. We therefore compute
the order-2 Shapley--Taylor interaction
index~\citep{sundararajan2020shapleytaylor}. With $n$ players and order
$k = 2$, singleton terms are the discrete derivative at the empty coalition,
$\mathcal{I}_g = v(\{g\}) - v(\varnothing)$, and each pair $\{a,b\}$ receives
%
\begin{equation}
\mathcal{I}_{ab} \;=\;
...
\begin{table}[t]
\caption{Order-2 Shapley--Taylor interaction indices on MovieLens-1M
(seed 42). Negative entries indicate redundancy. The two most negative pairs
are exactly the two overlaps declared in advance in
...
why leave-one-out scores it below zero. We note that $pop|cf$ was declared as
an expected overlap in advance (Section~\ref{sec:setup}), so this is
recovery of known structure rather than discovery. The interaction values are
...
ablation systematically undervalues. Three corpora do not support that: they
support the weaker and more useful claim that the two rules diverge on
\emph{some} source in every system, that the divergence tracks redundancy, and
that neither rule announces in advance where it will occur.
...
(Section~\ref{sec:segments}). The conclusion is equivalence, not superiority.
