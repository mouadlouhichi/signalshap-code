...
declared $0.60$ gate, only one corpus clears it.}
...
$C_u$ is nonempty. Their per-corpus frequencies are not available in the
paper-only artifact, so interpretations sensitive to repeated events remain
limited until those diagnostics accompany the synchronized release.
...
ordered in general. We measured the net effect with the head fitted in the
...
users, datasets, or deployments. Diagnostics that remain illustrative---the
estimand comparison and per-corpus robustness sweeps---use seed $42$ and are
...
interaction ordering does not rest on a single run. The values are
...
Brackets are Shapley run-to-run intervals.}
\label{tab:loo}
\small
\setlength{\tabcolsep}{2.5pt}
\begin{tabular}{@{}llrcrc@{}}
\toprule
Corpus & Source & $\LOOr$ & Shapley [run interval] & Gap & Shap. sign \\
\midrule
MovieLens-1M & $cf^{\dagger}$ & $-0.00457$ & $+0.01573\ [+0.01542,+0.01604]$ & $+0.02030$ & $10/10$ \\
             & $ct^{\ddagger}$ & $+0.00052$ & $-0.00016\ [-0.00024,-0.00008]$ & $-0.00068$ & $9/10$ \\
             & $pop$ & $+0.00070$ & $+0.00643\ [+0.00638,+0.00649]$ & $+0.00573$ & $10/10$ \\
             & $rec$ & $+0.00010$ & $+0.00032\ [+0.00022,+0.00042]$ & $+0.00022$ & $10/10$ \\
...
Amazon-VG    & $cf$ & $+0.00890$ & $+0.01980\ [+0.01937,+0.02022]$ & $+0.01089$ & $10/10$ \\
             ...
             & $rec^{\ddagger}$ & $+0.00014$ & $-0.00049\ [-0.00053,-0.00046]$ & $-0.00063$ & $10/10$ \\
...
Gowalla      & $cf$ & $+0.00743$ & $+0.00711\ [+0.00698,+0.00725]$ & $-0.00032$ & $10/10$ \\
             ...
             & $pop^{\dagger}$ & $-0.00156$ & $+0.00051\ [+0.00046,+0.00056]$ & $+0.00207$ & $10/10$ \\
...
that have not been regenerated under the symmetric rule are explicitly
...
recorded so this distinction is auditable. On
...
claim. The profiles remain useful for showing the scale of the variation that
the downstream fusion experiment attempted to exploit.
...
\url{https://github.com/mouadlouhichi/signalshap-code}. The public snapshot
available at the time of writing supports implementation inspection but does
not reproduce the present tables; this manuscript therefore makes no claim of
exact artefact reproducibility until a synchronized immutable release is
deposited.
...
\item \textbf{Code availability.} Public repository under the MIT license at
      \url{https://github.com/mouadlouhichi/signalshap-code}. The current public
      snapshot is not claimed to reproduce the present tables exactly; a
      synchronized immutable release remains required for that claim.
