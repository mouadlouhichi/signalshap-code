...
%% Figure 1 is strictly black and white. Structure is conveyed by rule weight
%% and dashed data-fold edges, with no colour or grey fill.
...
%% The Springer class defines thmstyleone/thmstyletwo only when amsthm has
%% already been loaded before the class, which is impossible in a normal
%% document.  Use amsthm's standard styles directly so the submission builds
%% without "Unknown theoremstyle" warnings.
\theoremstyle{plain}
...
\emph{sign} by a material margin. The disagreement persists in a labelled
seed-42 regularisation sweep. Under the evaluated five-source offline protocol,
ranking-stage leave-one-out has higher rank agreement with observed
single-source end-to-end removal losses on all three fixed corpora: mean
agreement is $0.96$--$1.00$ versus $0.20$--$0.80$ over ten stochastic fits, and
the observed cheapest source is identified on $90$--$100\%$ of fits versus
$0$--$50\%$.
...
Candidate recall is an applicability diagnostic: under our declared $0.60$
gate, only one corpus clears it.}
...
\node[snstage, text width=24mm, minimum height=25mm, inner sep=2pt] (src)
  {\textbf{Source models}\\[2pt]
   $cf,ct,pop$\\$rec,seq$};
\node[snstage, text width=24mm, minimum height=25mm, inner sep=2pt, right=of src] (cand)
  {\textbf{Fixed candidates}\\[2pt]
   $\Cu=\bigcup_g T_g(u,N_g)$\\{\snFigSub shared by all $S$}};
\node[snstagekey, text width=24mm, minimum height=25mm, inner sep=2pt, right=of cand] (game)
  {\textbf{Coalition game}\\[2pt]
   $2^5=32$ heads\\$w^{(S)},\ v(S)$};
\node[snstagekey, text width=24mm, minimum height=25mm, inner sep=2pt, right=of game] (shap)
  {\textbf{Shapley values}\\[2pt]
   $\varphi_g$ and $\varphi_g(u)$\\{\snFigSub exact aggregation}};

\draw[snflow] (src) -- (cand);
\draw[snflow] (cand) -- (game);
\draw[snflow] (game) -- (shap);

\node[snfold, text width=18mm, above=7mm of src] (train)
  {\textbf{Train}\\all but last two};
\node[snfold, text width=18mm, above=7mm of game] (val)
  {\textbf{Validation}\\second-to-last};
\node[snfold, text width=18mm, above=7mm of shap] (test)
  {\textbf{Test}\\last event};
\draw[sndata] (train) -- node[snlbl,right]{fit} (src);
\draw[sndata] (val) -- node[snlbl,right]{fit heads} (game);
\draw[sndata] (test) -- node[snlbl,right]{evaluate} (shap);

\node[snout, below=6mm of shap] (diag)
  {source credit $\bullet$ LOO $\bullet$ interactions};
...
\fi
\caption{SignalShap ranking-stage workflow and data-fold separation.}
\label{fig:workflow}
\end{figure}
...
%
\begin{equation}
\psi_g(v) \;=\; \sum_{k=0}^{n-1} \frac{p_k}{\binom{n-1}{k}}
...
\item \textbf{$rec$, recency over content clusters.} Items are assigned to one
      ...
      whenever the row is nonconstant. This argmax partition is not invariant to an
      arbitrary sign reversal of an SVD component. Reproduction therefore
      requires the exact fitted decomposition or an explicitly canonicalised
      component-sign convention; the implementation uses the cached fitted
      decomposition.
...
run-to-run intervals in the main text rather than an appendix. Efficiency, by
...
already appears in training, and the corpus-level repeat-event rate. Reporting
those counts is the precondition for interpreting the two cases above, and we
report them with the synchronized release rather than asserting they are
negligible: on check-in data in particular, repeat visits to the same venue are
expected, so Gowalla is the corpus where a non-trivial count is most likely.
...
(\texttt{scripts/run\_protocol\_sensitivity.py}) with the head fitted in the
...
\For{each user $u$}
  ...
  \State grow $\bigcup_gT_g(u,N_g)$ for at most $L$ passes as in
         Eq.~\eqref{eq:cand}; truncate by reciprocal-rank sum, then item index
  \If{$|C_u|=0$}
...
adds $O(m\,n\,N\log N)$. For the \emph{game}, accumulating the Gram matrix once
...
\[
\varphi_{g_1} = \varphi_{g_2} = -\tfrac{5}{12}\approx-0.4167,
\qquad \varphi_{g_3} = \tfrac43\approx1.3333,
...
$\varphi_{g_1} = 5/12\approx0.4167 \ge v(\{g_1\})/3 = 1/3$. Both games are encoded as
...
$\varphi_g = |\Ueval|^{-1}\sum_{u\in\Ueval} \varphi_g(u)$, by linearity of the
Shapley operator and the definition of $v$ in Eq.~\eqref{eq:game}.
\end{property}
...
\begin{table}[tbp]
\caption{Corpus and candidate-set preconditions. Users/eval. reports retained
and evaluated users; monotonicity entries are material/raw counts.}
...
Corpus & Users / eval. & Items & Density & $N_{\max}$ & Recall & Val.\ & Monot. & Prop.~\ref{prop:loo} \\
 &  &  &  &  & (test) & recall & (mat./raw) &  \\
\midrule
MovieLens-1M & 6\,038 / 6\,035 & 3\,533  & $2.697\%$ & 600    & $0.748$ & $0.799$ & $7$ / $23$ & no \\
Amazon-VG    & 7\,120 / 7\,120 & 3\,516  & $0.469\%$ & 600    & $0.588^{\dagger}$ & $0.648$ & $4$ / $12$ & no \\
Gowalla      & 8\,865 / 8\,865 & 82\,134 & $0.075\%$ & 11\,623 & $0.450^{\dagger}$ & $0.462$ & $6$ / $13$ & no \\
\botrule
\end{tabular}
\smallskip
...
reproducible statistic. On two machines, identical code and data produced
...
attribution barely moves: $\varphi$ goes from $(0.01926, 0.00187, 0.00157,
-0.00047, 0.01909)$ to $(0.01920, 0.00168, 0.00319, -0.00042, 0.01804)$, with
...
Recall is \emph{not} bounded below one: every unseen item receives a finite
score from every source, so setting $N_{\max}$ to the catalogue size makes
$C_u$ the whole eligible catalogue and recall approaches one by construction.
On MovieLens, $N_{\max}=3\,533$ yields recall $0.9995$. The logarithmic fit is
therefore interpreted only over the sampled range and is not extrapolated to
claim that the Gowalla gate is unreachable.
...
\caption{Per-source Shapley values over seeds 42--51. Whiskers are
$\bar{x}\pm t_9\mathrm{SE}$ run-to-run intervals.}
\label{fig:shares}
\end{figure}
...
$rec$ & content clusters      & $20$    & argmax over a $20$-component truncated
                                            ...
                                            $k$-means); \texttt{random\_state} $=$ seed \\
...
Game  & $N_{\max}$            & $600$ / $600$ / $11\,623$ & ML-1M / Amazon-VG / Gowalla \\
      & ridge $\lambda$       & $1.0^{\dagger}$ & summed-loss parameter; effective averaged-loss value is $1/|\mathcal U_{\rm fit}|$ \\
      & NDCG cutoff $K$       & $10$    & \\
...
frozen. All other values are fixed defaults; $N_{\max}$ alone varies by corpus.
\end{table}
...
\begin{equation}
\Delta_{\mathrm{red}}
= \max_{S \subseteq \G\setminus\{g,g_{\mathrm{dup}}\}}
\max\left\{
\begin{aligned}
&|v(S\cup\{g\})-v(S\cup\{g,g_{\mathrm{dup}}\})|,\\
&|v(S\cup\{g_{\mathrm{dup}}\})-v(S\cup\{g,g_{\mathrm{dup}}\})|
\end{aligned}
\right\}
\label{eq:reddev}
\end{equation}
...
\begin{table}[tbp]
\caption{Duplicate-source diagnostics on MovieLens-1M ($cf$ clone,
$\eta=0$).}
\label{tab:recovery}
...
\begin{table}[tbp]
\caption{Pairwise Shapley interactions on MovieLens-1M. Intervals describe
variation over ten stochastic fits.}
\label{tab:taylor}
\begin{tabular}{@{}lrcc@{}}
\toprule
...
\begin{table}[tbp]
\caption{Ranking-stage leave-one-out and Shapley attribution over ten seeds.
Brackets are run-to-run intervals, $\bar x \pm t_{0.975,9}\,s_x/\sqrt{10}$.
The quantity this paper contrasts is the paired gap
$\Delta_g = \varphi_g - \LOOr(g)$, so it carries its own interval, computed
per seed and then aggregated rather than differenced from the two marginal
means. The final column counts the fits on which the gap keeps the sign of its
mean: $14$ of $15$ cells are unanimous, the exception being $rec$ on
MovieLens-1M at $9/10$, whose gap is $0.4\%$ of $v(\G)$ and which the paper
does not interpret. Both material disagreements have gap intervals excluding
zero, at $38.8\%$ ($cf$, MovieLens-1M) and $12.2\%$ ($pop$, Gowalla) of the
respective $v(\G)$.}
\label{tab:loo}
\small
...
\end{tabular}
\smallskip
\noindent\footnotesize $^{\dagger}$Material sign flip;
$^{\ddagger}$sign flip below the $10^{-3}$ materiality threshold.
\end{table}

\begin{figure}[tbp]
\centering
\includegraphics[width=0.7\textwidth]{Fig3.png}
\caption{Ten-seed mean Shapley and ranking-stage leave-one-out attribution; the
diagonal denotes equality.}
\label{fig:scatter}
\end{figure}
...
\caption{Seed-42 sensitivity to candidate cap (top) and ridge penalty
(bottom); annotations report candidate recall.}
...
\caption{Pairwise source-score association across the three corpora
(Kendall $\tau$).}
...
Preconditions and duplicate test & 42 & legacy rule & sensitivity \\
Sweeps, ablations, and segments & 42 & legacy rule & exploratory \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{align}
v^{\mathrm{fixed}}_u(S)
  &= \NDCGat{K}\!\left(\operatorname{rank}(Z_u^{(S)}w^{(\G)}_S);
       \mathbf y_u^{\mathrm{te}}\right)-b_u,
       \label{eq:fixed-game}\\
r_u^{\mathrm{e2e}}(S)
...
U^{\mathrm{e2e}}_u(S)
  &=
\begin{cases}
\NDCGat{K}\!\left(r_u^{\mathrm{e2e}}(S);\mathbf y_u^{\mathrm{te}}(S)\right),
       & |C_u(S)|>0,\\
0, & |C_u(S)|=0.
\end{cases}\\
v^{\mathrm{e2e}}_u(S)
...
\label{eq:e2e-game}
\end{align}
Here $b_u(S)$ is Eq.~\eqref{eq:baseline} evaluated on $C_u(S)$ and is zero
when that set is empty. Aggregate values average these per-user quantities over
...
\begin{table}[tbp]
\caption{Three estimands compared at seed $42$, an estimand sensitivity
analysis rather than an inferential result. The refitted fixed-candidate head
is the paper's main game; the fixed head masks the deployed grand-coalition
head to $S$; the end-to-end game lets each coalition retrieve its own
candidates. $\tau$ is Kendall agreement with the main game's ordering}
\label{tab:estimands}
\small
...
\midrule
MovieLens-1M & refitted head (main) & $+0.01545$ & $-0.00009$ & $+0.00637$ & $+0.00026$ & $+0.02937$ & n/a \\
             & fixed head           & $+0.01867$ & $-0.00104$ & $+0.00528$ & $-0.00079$ & $+0.02924$ & $1.00$ \\
             & end-to-end           & $+0.01478$ & $-0.00010$ & $+0.00783$ & $-0.00022$ & $+0.02908$ & $0.80$ \\
\midrule
Amazon-VG    & refitted head (main) & $+0.01926$ & $+0.00187$ & $+0.00157$ & $-0.00047$ & $+0.01909$ & n/a \\
             & fixed head           & $+0.02110$ & $+0.00101$ & $+0.00166$ & $-0.00045$ & $+0.01799$ & $0.80$ \\
             & end-to-end           & $+0.01962$ & $+0.00133$ & $+0.00291$ & $+0.00026$ & $+0.01718$ & $0.80$ \\
\botrule
\end{tabular}
\end{table}
...
\begin{table}[tbp]
\caption{MovieLens-1M source-retirement loss at seed 42. Negative loss denotes
an NDCG@10 improvement after removal.}
...
\begin{table}[tbp]
\caption{Retirement agreement over ten training seeds. Intervals quantify
training-run variation on each fixed corpus.}
...
$p=0.0020$, $0.0039$, and $0.0020$ conditionally on treating seeds as
exchangeable runs. On Gowalla all ten differences equal $+0.20$, giving the
...
a_{s,g}
  &= (1-\rho)\,q_{s,g} + \rho\,q_{\mathrm{global},g}.
\label{eq:fuse}
\end{align}
%
...
\caption{Segment-level Shapley values; legends report segment sample sizes.}
\label{fig:segments}
\end{figure}
...
$\NDCG = 0.06327$ (Figure~\ref{fig:fusion}). These are descriptive differences,
...
which is practically negligible. The estimate is not stable in sign under the
plausible per-user normalisation in Eq.~\eqref{eq:head}, while its magnitude
remains within noise. \textbf{We therefore make no claim
...
\caption{Fusion comparison under full-catalogue evaluation. Amazon-VG and
Gowalla remain below the declared candidate-recall gate.}
\label{fig:fusion}
\end{figure}
...
The implementation, the frozen configuration, and every artefact behind a
reported number are available at
\url{https://github.com/mouadlouhichi/signalshap-code} at tag
\texttt{discover-ai-submission}. The repository ships
\texttt{artefacts/MANIFEST.json}, which records a SHA-256 for each of the $52$
artefact files together with the commit and the environment that produced them,
including the BLAS backend, since Section~\ref{sec:threats} shows that backend
changes candidate contents for a small number of users.
\texttt{artefacts/PROVENANCE.md} maps every table and figure to the artefact,
platform, candidate rule and seeds behind it, and names the diagnostics that
still come from the superseded candidate rule. One command per table and figure
is documented in the repository README. We state the residual limitation
precisely rather than in general terms: the raw corpora are not redistributed,
so a reader reproducing from data must first refetch them with the supplied
script, and the cross-platform residual means a run on a different BLAS
reproduces the reported signs and orderings but not every fifth decimal.
...
\item \textbf{Ethics approval.} Not applicable to new participant recruitment
      ...
      records. Popularity-based signals can also reinforce exposure disparities,
      and no fairness claim is made.
...
\item \textbf{Code availability.} Public repository under the MIT license at
      ...
      \texttt{discover-ai-submission}, with a hashed artefact manifest and a
      table-by-table provenance map. Reproduction from raw data requires
      refetching the corpora, which we do not redistribute, and exact decimal
      agreement is platform dependent as documented.
