...
cooperative game (collaborative filtering, content, popularity, recency and sequential order) whose payoff is a fixed-candidate normalised discounted
...
ranking-stage leave-one-out predicts the observed cost of removal \emph{better} than Shapley
on all three corpora and over ten seeds (mean rank agreement $0.96$--$1.00$
against $0.20$--$0.80$), identifying the cheapest source to retire on
$90$--$100\%$ of seeds against $0$--$50\%$. SignalShap allocates the value of a declared
ranking-stage game; leave-one-out estimates removal effect. We therefore
claim credit allocation, not retirement guidance, and identify candidate recall
as an applicability diagnostic under a declared $0.60$ gate that only one
corpus clears. Code and artefacts are
released.}
...
component exploits order. Engineering decisions, which component to invest in and which to retire, rest on estimates of how much each contributes. We show in
...
distinction is the subject of this paper. Leave-one-out is meaningful only
relative to the game on which it is computed, and this paper needs two versions
of it (Section~\ref{sec:background}). End-to-end leave-one-out removes a
component from retrieval and ranking together, so it \emph{is} the single-source
deployment intervention by definition. Ranking-stage leave-one-out removes it
only from the fixed-candidate ranker, and is a cheaper proxy that this paper
finds tracks the former closely. Either way, dependence between components does
not invalidate the estimand: removal cost is a legitimate thing to measure, as
our own experiments in Section~\ref{sec:estimands} confirm.
...
appear (Section~\ref{sec:related}). Our object of study is different: the players are
...
buys something unusual. Generic model-agnostic feature Shapley is usually approximated, because the
coalition lattice over hundreds of features is intractable, so KernelSHAP and
its relatives sample and inherit estimator variance; exact algorithms do exist
for particular model classes, trees being the standard
example~\citep{lundberg2020}. With
five architectural sources the lattice has $2^5 = 32$ elements: the whole game
needs $32$ coalition evaluations, and each source's Shapley value is then a sum
of the $2^4 = 16$ marginal contributions in which that source is the one added.
We enumerate both exactly. Second, and more
...
\item \textbf{C3.} A validation protocol for source attribution combining
      ...
      that the aggregation is implemented correctly, a precondition for interpreting anything computed with it, and one the field rarely checks.
\item \textbf{C4 (primary).} An empirical comparison of three estimands (refitted-head ranking-stage, fixed-head, and end-to-end retrieval), showing that Shapley allocation and leave-one-out removal cost answer
      ...
      agreement $0.96$--$1.00$ against $0.20$--$0.80$) and identifies the
      ...
      declared $0.60$ reporting gate, and report where our
...
attribution prescribes retaining or removing a source. Section~\ref{sec:estimands} shows it does not.
...
that has nothing to do with the observed change in ranking quality. A reviewer
caught us defining $\LOOe$ on the centred game and calling the result the raw
removal cost; the two differ, and the definition above is the corrected one.
Section~\ref{sec:estimands} reports the size of the term we had been carrying.
...
\subsection{What each source computes}\label{sec:sources}

A reviewer noted that naming the five players and listing their
hyperparameters does not let anyone reimplement them. We therefore give the
scoring function of each. Write $R \in \{0,1\}^{m \times |\mathcal{I}|}$ for the
...
We must flag one thing about those two numbers, because a reviewer reasonably
read them as contradicting the main results, where the same seed-42
...
A reviewer asked what history the test ranking is produced from, and the
question deserves an explicit answer because the split is leave-last-\emph{two}-out.
...
consequence is directional and one is not, and an earlier version of this
manuscript conflated them.
...
We must correct a statement made in an earlier version. Such a user contributes
its positive-semidefinite block to $A$ but nothing to $c$, and we described that
as shrinking the fitted weights without distorting their direction. That is
false. For $w = (A + \lambda I)^{-1} c$ with $c$ unchanged, adding a PSD term to
...
and is not globally time-blocked, a distinction a reviewer asked us to make
explicit. Because every source is fitted on the pooled training fold, and
...
blocked replication would need a single cutoff date and would discard a large
share of each corpus; we have not run one.
...
ten-seed means and intervals; an earlier version described them as a single-run
diagnostic and that is no longer true. Diagnostics that remain illustrative
...
The practical reading needs care, and an earlier version overstated it by
saying an engineer would retire both duplicates and lose the capability. A
...
\paragraph{Other semivalues, and a correction}
The Shapley value is one member of a family. An earlier version of this
manuscript reported a third comparator it called a ``uniform semivalue'',
defined by
$\psi_g = \tfrac1n\sum_{k=0}^{n-1}\binom{n-1}{k}^{-1}
 \sum_{|S|=k,\,S\subseteq\G\setminus\{g\}}[v(S\cup\{g\})-v(S)]$,
and asserted that it does not satisfy efficiency. A reviewer pointed out that
this expression is algebraically identical to Equation~(\ref{eq:shapley}),
since $|S|!(n-|S|-1)!/n! = 1/\bigl(n\binom{n-1}{|S|}\bigr)$. The reviewer is
right, and our artefact confirms it numerically: the two vectors agreed to
$0.0$ in every coordinate on every corpus, which we had read as robustness when
it was an identity. That comparator carried no information and the efficiency
claim about it was false. We withdraw both.

The distinct comparators we now report are Banzhaf, $p_k =
...
computing the Grabisch--Roubens coefficient. The two are not interchangeable, since at $n=5$ their coalition weights differ by up to a factor of $2.5$, so the label was wrong even though the computation was a valid
...
structure, and note that an earlier version of this paper overstated it. Every
...
the same aggregation level; an earlier version mixed a seed-42 point estimate
with a ten-seed interval. Intervals are $\bar{x} \pm t_9\,\mathrm{SE}$, and no
...
are ten-seed means over seeds $42$--$51$, matching Table~\ref{tab:loo}; an
earlier version plotted seed-42 points under ten-seed prose. Points on the
...
$0.00063$, with both estimates within noise of zero. We state this plainly because
...
earlier version wrongly listed them alongside the theorems. Neither is proved.
...
leading pair exchanges. An earlier version of this manuscript claimed
the frozen values were a lower bound on the refreshed ones. The measurement
agrees with that direction on all three corpora, but the claim was still unsound as
stated, since it ignored the simultaneous changes to the source scores, the
candidate set and the baseline; and it distracted from the finding that
actually matters, which is the ordering instability on Amazon-VG.
...
\paragraph{The ordering does not depend on the choice of ranking metric}
NDCG@10 is the only metric in the characteristic function, so a reviewer asked
whether the attributions are an artefact of it. We re-scored the same game with
...
Equation~(\ref{eq:loo}). An earlier version differenced the centred
$v_{\mathrm{e2e}}$ instead, which silently added
$b(\G\setminus\{g\}) - b(\G)$ to every reported cost. We recomputed both. The
...
reaches $6.7\times10^{-4}$, four times the bound we had estimated from a pilot,
which is why the correction was worth making rather than waiving. It changes
nothing that the paper concludes from: $cf$ remains the cheapest source to
retire, $\tau_{\LOOr} = 1.00$ and $\tau_{\text{Shapley}} = 0.20$ as before. An
earlier version
reported a constant $+0.00038$ offset here, which was an artefact of the
sampled baseline that version used and no longer exists. Amazon-VG reproduces
...
weakest. An earlier version reported $p = 1.0$ there, from a guard that treated
constant differences as uninformative; that was wrong. With $n = 10$ all
...
Reporting two intervals side by side does not test their difference, and a
reviewer was right to ask for the paired contrast: $\tau_{\LOOr}$ and
...
conflated them. $\LOOe$ is the right instrument for ``what happens if I remove this
...
legend; the counts differ by corpus, and an earlier version showed a single
shared legend carrying only Gowalla's. On MovieLens the four segments cover the
...
ranking across corpora, so an earlier version added a Friedman omnibus over the
six methods blocked by corpus with a Nemenyi post-hoc. We have removed it. The
...
applicability rule. The previous version retained it on the grounds that its
conclusion was negative and therefore harmless. A reviewer objected that
admissibility cannot depend on the outcome, which is correct: a rule applied
only when it is convenient is not a rule. For the record, the omnibus was
...
\item \textbf{Data availability.} MovieLens-1M is used under the GroupLens
      ...
      and none is redistributed by us: \texttt{scripts/fetch\_benchmarks.sh}
      retrieves each from its original source under that source's own terms.
      The artefact JSON backing every reported number is in the repository.
\item \textbf{Code availability.} Public repository under the MIT license, at
      \url{https://github.com/mouadlouhichi/signalshap-code}, commit
      tag \texttt{discover-ai-submission}.
\item \textbf{Author contributions.} \textbf{M.\ Louhichi:} conceptualisation,
      ...
      supervision, methodology, resources, writing (review and editing). \item \textbf{Use of AI tools.} In line with Springer Nature's guidance on
      generative AI, we disclose that large language models were used for code
      ...
      verification, and no AI system is listed as an author. All mathematical
