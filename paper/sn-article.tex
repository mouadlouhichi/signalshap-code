...
We ran one rather than leave the question open
(\texttt{scripts/run\_global\_timeblock.py}). Splitting MovieLens-1M at the
$0.90$ and $0.95$ global timestamp quantiles gives folds in which \emph{no}
training event postdates any evaluated event, which we verify rather than
assume. The price is severe: users with no interaction after the cutoff drop
out entirely, so $526$ of $6\,038$ users survive, $8.7\%$, and the survivors
skew heavy and late-active. Candidate recall falls to $0.464$, below the
$0.60$ gate this paper declares in Section~\ref{sec:preconditions}, and
$v(\G)$ falls from $0.0522$ to $0.0095$.

We report the outcome even though it does not flatter the paper. The
allocation ordering is only partly preserved (Kendall $\tau = 0.60$ against
the main ten-seed ordering; $cf$ and $seq$ exchange the top two places), and
$\LOOr$ reorders more than that ($\tau = -0.40$). The material sign
disagreement moves as well: under the blocked split it appears on $pop$ and
$seq$ rather than on $cf$. We therefore do \emph{not} claim that the
per-source attributions replicate under a globally causal split. What the
replication does show is that the qualitative phenomenon the paper is about,
a material sign disagreement between allocation and ranking-stage removal,
still occurs when every trace of cross-user future information is removed;
it lands on different sources.

Two things prevent us from reading more into it. The blocked corpus fails the
paper's own applicability gate, so by our stated policy it supports relative
contrasts only. And at $8.7\%$ coverage it is a different population, not a
cleaner measurement of the same one, which is the reason the main study uses
the per-user protocol that the recommender literature it compares against also
uses. A corpus dense enough to survive a global cutoff while clearing the
recall gate would settle this properly, and we do not have one.
...
than a cleaner measurement of the same one. The honest position is that our
per-source numbers describe offline per-user chronological evaluation, which is
what the recommender literature this work compares against also does, and that
we have not shown they transfer to a globally causal deployment simulation.
...
\item \textbf{Code availability.} Public repository under the MIT license, at
      \url{https://github.com/mouadlouhichi/signalshap-code}, tag
      \texttt{discover-ai-submission}. Every artefact behind a reported number
      is released with it.
\item \textbf{Use of AI tools.} In line with Springer Nature's guidance on
      ...
      verification, and no AI system is listed as an author.
\end{itemize}
