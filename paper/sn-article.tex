...
\begin{table}[tbp]
\caption{Repeat-event audit (\texttt{scripts/audit\_repeat\_items.py}).
Percentages are of users holding both folds. ``Test item in train'' users are
masked out of every candidate set, so their per-user payoff is zero for all
$32$ coalitions.}
\label{tab:repeats}
\small
\begin{tabular}{@{}lrrrr@{}}
...
than a cleaner measurement of the same one. Because the paper's operational
claim is specifically about retirement, the released pipeline runs the
end-to-end retirement simulation under the blocked split as well
(\texttt{run\_global\_timeblock.py -{}-retirement}), so the
$\tau_{\LOOr}$-versus-$\tau_{\text{Shapley}}$ contrast can be checked on a
causal split and not only the attribution vectors. The honest position is that
our per-source numbers describe offline per-user chronological evaluation,
which is what the recommender literature this work compares against also does,
and that we have not shown they transfer to a globally causal deployment
simulation.
...
matters is the ordering instability on Amazon-VG. Those figures are seed-42
single runs, which cannot separate a protocol effect from run-to-run noise;
the released pipeline therefore also runs the contrast across the same ten
seeds as the main results (\texttt{run\_protocol\_sensitivity.py -{}-seeds}),
reporting a paired percentile-bootstrap interval on the change in $v(\G)$ and
a per-source sign count, so the comparison rests on an interval rather than on
one draw.
