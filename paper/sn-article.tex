...
\abstract{Hybrid recommender systems fuse architecturally distinct signals ---
collaborative filtering, content, popularity, recency, and sequential order ---
and practitioners need to know how much each earns. We present
\emph{SignalShap}, which recasts source attribution as a five-player
cooperative game whose payoff is a fixed-candidate normalised discounted
...
coalitions, so the aggregation carries no sampling error.
...
SignalShap recovers this with efficiency to $6.9\times10^{-18}$. Six analytic
games with closed-form Shapley vectors are reproduced exactly, validating
magnitudes rather than symmetry alone.

Across three timestamped corpora spanning a $36\times$ density range, every
system contains at least one source on which leave-one-out and Shapley disagree
in \emph{sign}. Which source flips is corpus-specific: collaborative filtering
on MovieLens-1M ($-0.0050$ against $+0.0155$), popularity on Gowalla, recency
on Amazon. Shapley--Taylor indices localise the redundancy driving this.

Two findings bound the practical claim. In an end-to-end retirement simulation
leave-one-out predicts the true cost of removing a source \emph{better} than
Shapley ($\tau=1.00$ versus $0.20$ and $0.60$), so we claim credit allocation
rather than retirement guidance. On Gowalla the candidate-recall gate proves
unreachable at any pool size, locating a sparsity boundary for fixed-candidate
attribution. Code and every backing artefact are released.}
...
\item \textbf{C3 (primary).} A \emph{diagnostic protocol} for source
      ...
      the method meets it to numerical precision
...
\item \textbf{C3b.} Evidence that the credit SignalShap assigns is
      ...
      the direction the redundancy structure predicts. On all three corpora at
      least one source receives opposite signs from the two methods; the
      largest case is MovieLens-1M, where collaborative filtering is the second
      most valuable source ($+0.0155$) and ablation scores it below zero
      ($-0.0050$), a gap of $39\%$ of the grand-coalition payoff. We report
      that \emph{which} source flips varies by corpus --- popularity on
      Gowalla, recency on Amazon-VG, the latter negligible in magnitude --- so
      the claim is that ablation misjudges some source in every system, not
      that it systematically misjudges a particular one
...
\midrule
MovieLens-1M & 6\,038 & 3\,533  & $2.697\%$ & 600    & $0.748$ & $16/80$ & no \\
Amazon-VG    & 7\,120 & 3\,516  & $0.469\%$ & 600    & $0.588^{\dagger}$ & $12/80$ & no \\
Gowalla      & 8\,865 & 82\,134 & $0.075\%$ & 11\,623 & $0.450^{\dagger}$ & $13/80$ & no \\
\botrule
\end{tabular}
\smallskip
...
weights worse for some users, and with $\NDCG$ at a cutoff of ten this is
visible in $12$--$16$ of the $80$ audited pairs. It is the reason the
monotonicity hypothesis was made explicit in Section~\ref{sec:theory} instead
of assumed.
...
the pool buys $+0.021$. Extrapolating that logarithm, recall reaches only
$0.522$ at $N_{\max}=82\,134$ --- \emph{ranking the entire catalogue} --- and
the $0.60$ crossing sits at $N_{\max}\approx 624\,000$, some $7.6\times$ more
items than exist.

The gate is therefore unreachable on Gowalla at any pool size, and the binding
constraint is not the pool: it is the coverage of the five base scorers'
top-lists. This is worth stating plainly because it locates a boundary on the
method rather than on the implementation. Fixed-candidate attribution requires
that a union of source top-$N$ lists can cover a usable fraction of held-out
interactions; on an $82$k-venue check-in corpus with $0.075\%$ density, five
scorers cannot. A practitioner can check this condition before adopting the
method, and we would rather report where it fails than select corpora where it
does not.
...
\begin{table}[t]
\caption{LOO versus Shapley on all three corpora (seed 42). Every corpus
contains at least one source on which the two methods disagree in \emph{sign};
the source that flips differs by corpus, and so does the magnitude. Seed means
and confidence intervals are in the artefact.}
\label{tab:loo}
\begin{tabular}{@{}llrrrl@{}}
\toprule
Corpus & Source & LOO & Shapley $\varphi_g$ & Gap & \\
\midrule
MovieLens-1M & $cf$  & $-0.00501$ & $+0.01547$ & $+0.02048$ & \textbf{sign flip} \\
             & $ct$  & $+0.00083$ & $+0.00008$ & $-0.00075$ & \\
             & $pop$ & $+0.00098$ & $+0.00664$ & $+0.00565$ & \\
             & $rec$ & $+0.00044$ & $+0.00071$ & $+0.00027$ & \\
             & $seq$ & $+0.01673$ & $+0.02964$ & $+0.01290$ & \\
\midrule
Amazon-VG    & $cf$  & $+0.00845$ & $+0.01952$ & $+0.01107$ & \\
             & $ct$  & $+0.00002$ & $+0.00213$ & $+0.00211$ & \\
             & $pop$ & $+0.00308$ & $+0.00183$ & $-0.00125$ & \\
             & $rec$ & $+0.00023$ & $-0.00021$ & $-0.00043$ & \textbf{sign flip} \\
             & $seq$ & $+0.00758$ & $+0.01935$ & $+0.01178$ & \\
\midrule
Gowalla      & $cf$  & $+0.00729$ & $+0.00712$ & $-0.00017$ & \\
             & $ct$  & $+0.00528$ & $+0.00675$ & $+0.00147$ & \\
             & $pop$ & $-0.00151$ & $+0.00057$ & $+0.00207$ & \textbf{sign flip} \\
...
\paragraph{What is consistent}
Every corpus contains at least one source on which LOO and Shapley disagree in
sign --- that is, one method scores the source as contributing and the other as
actively harmful. Since a sign determines an action (retain or remove), these
are not differences of degree that careful reading would survive. Ablation and
Shapley recommend opposite decisions about some component of every hybrid we
examined.

\paragraph{What is not}
Which source flips is corpus-specific, and so is how much it matters. On
MovieLens the flip falls on $cf$ and the gap is $0.0205$, or $39\%$ of $v(\G)$
--- the second most valuable source in the system, scored by ablation as worth
removing. On Gowalla it falls on $pop$, at $12\%$ of $v(\G)$. On Amazon-VG it
falls on $rec$, at $1\%$ of $v(\G)$, where both estimates are within noise of
zero and the disagreement is real but inconsequential. A single-corpus reading
of this paper would have concluded that collaborative filtering is the source
ablation systematically misjudges. Three corpora do not support that: they
support the weaker and more useful claim that redundancy-blind ablation
misjudges \emph{some} source in every system, and does not announce which.
...
between corpora is $0.20$, $0.40$, and $0.80$, none significant at $n=5$
sources. We therefore make no cross-corpus consistency claim. An earlier
...
removal \emph{improved} the metric. Amazon-VG reproduces the pattern
...
on Gowalla we show that no pool size can lift it: that locates a sparsity
boundary for fixed-candidate attribution rather than a shortfall of effort.
