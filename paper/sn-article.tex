...
sequential order --- and practitioners need to know how much each earns.
We present \emph{SignalShap}, which recasts source attribution as a
five-player cooperative game whose payoff is a fixed-candidate normalised
discounted cumulative gain at rank~10 (NDCG@10) on a coalition-independent
candidate set. With five players the Shapley value follows from exact
enumeration of all $2^5=32$ coalitions, so the aggregation carries no sampling
error.

We validate the implementation against constraints known in advance: duplicate
injection makes a pair exchangeable, so symmetry requires equal credit, and
SignalShap recovers this to numerical precision with efficiency to
$6.9\times10^{-18}$. Six analytic games with closed-form Shapley vectors ---
additive, substitute, complementary, null, and harmful sources --- are
reproduced exactly, validating magnitudes rather than symmetry alone. These
remain implementation checks, not evidence of correct magnitudes.

Two results bound the practical claim. On MovieLens-1M, leave-one-out reports
collaborative filtering as harmful ($-0.0050$) where Shapley ranks it second
most valuable ($+0.0155$); second-order Shapley--Taylor indices localise the
redundancy driving this. But in an end-to-end retirement simulation
leave-one-out predicts the true cost of removing a source \emph{better} than
Shapley ($\tau=1.00$ versus $0.20$). The two answer different questions, and we
claim credit allocation rather than retirement guidance. Fusion weights from validation-fold attributions beat LightGCN, SASRec,
uniform, and popularity baselines at $p<0.001$, but not a tuned global head.
Results use one corpus and three seeds; code is released.}
...
\item \textbf{C3b.} Evidence that the credit SignalShap assigns is
      ...
      the direction the redundancy structure predicts: on MovieLens-1M it identifies
...
\item \textbf{C4b.} A comparison of three estimands (fixed head, refitted
      head, end-to-end retrieval) and a retirement simulation showing that
      leave-one-out, not Shapley, is the appropriate instrument for
      single-source removal decisions (Section~\ref{sec:estimands}). This
      narrows the paper's practical claim, and we regard stating it as part of
      the contribution.
...
\item[RQ3] Do per-user attributions vary across behavioural segments, or does
  the global summary suffice?
\item[RQ4] Do fusion weights derived from segment-level attributions improve
  ranking quality relative to uniform, neural, and globally tuned baselines?
\end{description}
...
defined to be that same permutation. Note that $\varphi_g$ is invariant to the
baseline altogether, since $b_u$ cancels in every marginal difference
$v_u(S\cup\{g\}) - v_u(S)$: it exists only so that efficiency reads
$\sum_g \varphi_g = v(\G)$ rather than $v(\G) - v(\varnothing)$.
...
\midrule
$cf$  & $-0.00501$ & $+0.01547$ & $+0.02048$ & \textbf{sign flip} \\
$ct$  & $+0.00083$ & $+0.00008$ & $-0.00075$ & \\
$pop$ & $+0.00098$ & $+0.00664$ & $+0.00565$ & \\
$rec$ & $+0.00044$ & $+0.00071$ & $+0.00027$ & \\
$seq$ & $+0.01673$ & $+0.02964$ & $+0.01290$ & \\
\botrule
\end{tabular}
\end{table}
...
$-0.0050$ --- it looks \emph{actively harmful}, and an engineer following the
standard methodology would retire it. Shapley scores the same component at
$+0.0155$, the second most valuable source in the system. The disagreement is
...
Measured redundancy is associated with the attribution shift at $r = 0.525$
...
Resolving a difference of this magnitude would require on the order of $10^5$
users against the $6\,035$ available. Rather than describe the result as
underpowered, we state the defensible conclusion directly: at this scale the
two are \emph{equivalent for practical purposes}, and establishing otherwise
would require a pre-specified smallest meaningful difference and a formal
equivalence test, which we did not conduct. We therefore make the claim the evidence supports: SignalShap
...
end-to-end). The ranking-stage restriction is therefore consequential in
principle but does not change the ordering here --- a scope limitation we can
now quantify rather than merely declare.
...
\begin{table}[t]
\caption{True cost of retiring each source on MovieLens-1M, measured
end-to-end, against what each attribution method predicts. Negative loss means
removal \emph{improved} the metric}
\label{tab:retirement}
\begin{tabular}{@{}lrrrr@{}}
\toprule
Source & True loss & Recall after & Shapley & LOO \\
\midrule
$cf$  & $-0.00561$ & $0.690$ & $+0.01547$ & $-0.00501$ \\
$rec$ & $-0.00082$ & $0.782$ & $+0.00071$ & $+0.00044$ \\
$ct$  & $+0.00002$ & $0.785$ & $+0.00008$ & $+0.00083$ \\
$pop$ & $+0.00247$ & $0.723$ & $+0.00664$ & $+0.00098$ \\
$seq$ & $+0.01569$ & $0.659$ & $+0.02964$ & $+0.01673$ \\
\midrule
...
(Table~\ref{tab:retirement}).} Its ranking matches the true loss exactly
($\tau = 1.00$ against $0.20$), and it correctly identifies $cf$ as the
cheapest source to retire, where Shapley selects $ct$. We report this plainly
...
substantially redundant with $seq$ and $pop$ --- the Shapley--Taylor interaction
