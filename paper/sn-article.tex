...
$80$--$100\%$ of seeds against $0$--$50\%$. SignalShap allocates the value of a declared
...
$z$-normalised per user over $\Cu$ before it enters the game, so only the
induced ordering matters.
%
\begin{itemize}
...
\item \textbf{$pop$, time-decayed global popularity.} With half-life
      ...
      2^{-(t_{\max}-t)/(86400 h)}\bigr)$. It does not depend on $u$, so it is
      constant across a user's candidate row and contributes only through
...
(timestamp, record index). Ten seeds ($42$--$51$) are used for every result
...
test recall on both corpora where we measured it, because the validation item
is one step closer to the training history. Gowalla's is not yet computed and
...
index is negative. Negative entries indicate redundancy.
...
source in the system, where $\LOOr$ scores it $-0.00457$ and marks it as worth
removing. The gap is $0.02030$ $[+0.02007, +0.02053]$, or $38.8\%$ of the
...
Amazon-VG has no material disagreement. Its only sign flip, on $rec$, is $0.0008$, with both estimates within noise of zero. We state this plainly because
...
w^{(S)});\, \mathbf{y}^{\mathrm{te}}_u\bigr) - b_u(S)$. A removed source
therefore loses its retrieval role too, and coalition values are no longer
comparable across a fixed item set, which is exactly the property the main game
is designed to preserve. On
...
same game as Table~\ref{tab:loo} and are identical to it; an earlier version
...
intervals at $n = 10$. The $\tau$ intervals are $\bar\tau \pm t_9\,\mathrm{SE}$
over the ten seed-level correlations, matching the convention used everywhere
else in the paper, and are degenerate where every seed returns the same value.
$\Delta\tau$ is the seed-paired difference $\tau_{\LOOr} -
\tau_{\text{Shapley}}$ with a percentile interval from $10\,000$ bootstrap
resamples of the ten differences; Wilcoxon signed-rank gives $p = 0.002$
(MovieLens) and $p = 0.004$ (Amazon-VG), while on Gowalla every difference is
identical so the test has no rank variation to use and returns $p = 1.0$
uninformatively. With $n = 10$ all intervals here are indicative rather than
calibrated}
...
Corpus & $\tau$ $\LOOr$ & $\tau$ Shapley & Paired $\Delta\tau$
       ...
       & ($t_9$ CI) & ($t_9$ CI) & (bootstrap CI) & (Wilson) & (Wilson) \\
\midrule
MovieLens-1M & $0.96\ [0.90, 1.02]$ & $0.20\ [0.20, 0.20]$
             & $+0.76\ [+0.70, +0.80]$
             & $100\%\ [72, 100]$ & $0\%\ [0, 28]$ \\
Amazon-VG    & $0.96\ [0.90, 1.02]$ & $0.74\ [0.67, 0.81]$
             & $+0.22\ [+0.16, +0.28]$
             & $90\%\ [60, 98]$   & $50\%\ [24, 76]$ \\
...
$p = 0.002$ and $p = 0.004$ on the first two corpora; on Gowalla the test is
uninformative ($p = 1.0$) precisely because every difference is identical, so
there is no rank variation for it to use, and the zero-width interval is the
honest summary there. Intervals are percentile bootstraps over $10\,000$
...
Remove one deployed source
  ...
  & $\tau = 0.94$--$1.00$ over 10 seeds (Sec.~\ref{sec:estimands}) \\[2pt]
...
difference is $+0.00003$ ($d_z = 0.001$, Holm-corrected $p = 0.77$), indistinguishable from zero. The sign of this comparison has flipped between
