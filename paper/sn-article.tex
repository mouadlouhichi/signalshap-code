...
Leave-one-out ablation reports $\LOO(g) = v(\G) - v(\G\setminus\{g\})$: the
single marginal contribution at the grand coalition. It is a valid estimand for
the effect of removing $g$ while everything else remains, and it is not an
approximation to the Shapley value; it is one of the $2^{n-1}$ terms the
Shapley value averages.
...
genuinely cheapest source to retire}
...
suggest: LOO identifies the genuinely cheapest source to retire on $100\%$,
...
\midrule
Remove one deployed source & end-to-end LOO & $\tau = 0.96$--$1.00$, 10 seeds (Section~\ref{sec:estimands}) \\
...
genuinely cheapest source to retire. It remains a result about five-source
...
\paragraph{A multi-comparison test the per-corpus tests cannot replace}
Both reviewers noted that a Wilcoxon test per corpus establishes a difference
on that corpus, not a ranking across corpora. We therefore add a Friedman
omnibus over the six methods, blocked by corpus, with a Nemenyi post-hoc. The
omnibus is significant ($\chi^2 = 14.24$, $p = 0.014$) and the mean ranks order
the methods as one would expect --- SignalShap-Fuse $1.33$, globally tuned head
$1.67$, uniform $3.00$, LightGCN $4.00$, popularity $5.33$, SASRec $5.67$ ---
but with three blocks the critical difference is $4.35$ and \emph{no pair is
separable}. We report this because it is the honest reading: our
per-corpus $p < 0.001$ results against uniform and the neural references are
real differences on those corpora, but they do not license a claim that the
methods rank reliably across corpora. Three blocks is too few, and adding
corpora, not seeds, is what would fix it.
