...
by a material margin; the third disagrees only negligibly. Which source
diverges is corpus-specific. Yet in an end-to-end retirement simulation,
leave-one-out predicts the observed cost of removal \emph{better} than Shapley
($\tau=1.00$ against $0.20$ and $0.60$). SignalShap allocates the value of a
declared ranking-stage game; leave-one-out estimates the effect of removal from
the deployed system. We therefore claim credit allocation, not retirement
guidance, and identify candidate recall as an applicability test that only one
of our three corpora passes. Code and artefacts are released.}
...
\midrule
Remove one deployed source & end-to-end LOO & $\tau = 1.00$ (Section~\ref{sec:estimands}) \\
...
LOO ranks retirement cost perfectly on both corpora where we could measure it.
With five sources per corpus the exact two-sided permutation $p$ for
$\tau = 1.00$ is $0.017$, so a single corpus would be suggestive rather than
conclusive; two independent replications make it substantially stronger. It
remains a result about five-source hybrids under one deployment protocol, and
we would not extrapolate it to systems with many more components, where the
grand coalition is a less special point in the lattice.
