...
so frozen and refreshed $v(S)$ are not ordered in general. The honest statement
is that this is a different protocol whose net effect we have not measured, and
that the sources a refreshed state would most plausibly favour, $seq$ and
$rec$, are precisely the ones whose values should be read with that in mind. A
frozen-versus-refreshed sensitivity run is the experiment that would settle it.
...
The effect is large. Refreshing raises $v(\G)$ from $0.05169$ to $0.07026$ on
MovieLens-1M and from $0.04130$ to $0.04902$ on Amazon-VG, increases of $36\%$
and $19\%$. On these two corpora the frozen values do therefore sit below the
refreshed ones, which is the direction the withdrawn claim asserted. We still
withdraw it, because it was asserted as a general property and it does not
follow: two measurements agreeing with an argument do not repair an argument
that ignores the accompanying changes to the source scores, to $\Cu$ and to
$b_u$. What we can say is empirical and corpus-specific, not a bound. Candidate
recall
also rises, $0.748 \to 0.779$ and $0.589 \to 0.604$, because a refreshed
history retrieves better. The gain is concentrated exactly where we predicted
...
What survives is the ordering, and only mostly. On MovieLens the source ranking
is identical under both protocols (Kendall $\tau = 1.00$), with the sole sign
change on $\varphi_{ct}$ between $+0.00002$ and $-0.00002$, a magnitude the
paper never interprets. On Amazon-VG the ranking moves ($\tau = 0.80$) and the
...
concerns: $0.799$ on MovieLens-1M, where $1\,216$ of $6\,035$ fitted users
contribute negatives only, and $0.648$ on Amazon-VG, where $2\,505$ of
$7\,120$ do. Those are large minorities, so the policy needed testing rather
...
MovieLens with an identical ordering, and $2.1\times10^{-4}$ on Amazon-VG,
where the ordering shifts only among $ct$ and $pop$, which are separated by
$3\times10^{-4}$. Every sign agrees under both policies on both corpora. The
...
validation candidate recall bounds the fit in the same way; it is higher than
test recall on both corpora where we measured it, which is consistent with the
validation item lying one step closer to the training history, though we did
not test that explanation. Gowalla's is not yet computed and
is marked \emph{n/a}. Section~\ref{sec:algorithm} reports how many users
...
Gowalla      & 8\,865 & 82\,134 & $0.075\%$ & 11\,623 & $0.450^{\dagger}$ & n/a & $6$ / $13$ & no \\
\botrule
\end{tabular}
\smallskip
...
The ten-seed attribution and retirement results, which carry every inferential
claim, were regenerated on all three corpora under the source-symmetric key of
Section~\ref{sec:game}. Two single-seed diagnostics were not: the
duplicate-injection experiment of Section~\ref{sec:recovery} and the estimand
comparison of Section~\ref{sec:estimands} still come from the superseded
order-dependent key. Both are structural checks whose conclusions are about
exact symmetry and rank agreement rather than about particular values, and on
MovieLens the two keys agree to $\max_g|\Delta\varphi_g| = 2.8\times10^{-5}$
with identical ordering; we nonetheless flag them rather than imply the whole
paper is single-key. A machine-readable map from every table and figure to the
...
about it, and it is substantial: a refreshed one-step-ahead protocol raises
$v(\G)$ by $36\%$ on MovieLens-1M and $19\%$ on Amazon-VG, most of it accruing
to $seq$. The absolute coalition values reported here are therefore specific to
the frozen protocol and understate what these sources achieve with a current
history. The source ordering is unchanged on MovieLens but not on Amazon-VG,
where the leading pair exchanges. An earlier version of this manuscript claimed
the frozen values were a lower bound on the refreshed ones. The measurement
agrees with that direction on both corpora, but the claim was still unsound as
...
ten-seed means. One caveat on the observed-loss column: it was produced before
the raw-utility correction of Section~\ref{sec:background} and therefore
differences the baseline-centred $v_{\mathrm{e2e}}$, which adds a term bounded
by $1.7\times10^{-4}$ on our measurements. The ranking, the cheapest source and
both $\tau$ values are unaffected, and the ten-seed retirement results in
Table~\ref{tab:retire-seeds} do use the corrected definition; only the
fourth and fifth decimals of this single-seed illustration are provisional. An
earlier version
...
\midrule
$cf$   & $-0.00542$ & $0.689$ & $+0.01545$ & $-0.00519$ \\
$rec$  & $-0.00135$ & $0.782$ & $+0.00026$ & $-0.00005$ \\
$ct$   & $-0.00075$ & $0.783$ & $-0.00009$ & $+0.00044$ \\
$pop$  & $+0.00176$ & $0.719$ & $+0.00637$ & $+0.00065$ \\
$seq$  & $+0.01578$ & $0.657$ & $+0.02937$ & $+0.01635$ \\
\midrule
...
end-to-end metric by $0.00542$. We write \emph{observed} rather
...
it. Removing $cf$ \emph{improves} end-to-end NDCG@10 by $0.00542$ while
dropping candidate recall from $0.748$ to $0.689$: the collaborative signal is
