...
\title[Verified Source-Level Credit Attribution]{SignalShap: Verified
Source-Level Credit Attribution for Hybrid Recommenders}
...
Our central result is a \emph{validation}: we establish that the method
recovers correct credit on real data where the answer is known in advance.
Injecting a controlled duplicate of an existing source fixes the correct
attribution by construction, since symmetry requires the duplicated pair to
receive equal credit. SignalShap recovers this to numerical precision on
\emph{all four} corpora, which span a $49\times$ density range, while the
leave-one-out ablation that dominates current practice returns zero credit for
both members of every pair. Efficiency is verified to $6.9\times10^{-18}$, and
measured redundancy tracks the attribution shift at $r=0.49$, recovering
structure we pre-registered as present. Fusion weights derived from these
attributions outperform LightGCN, SASRec, uniform, and popularity baselines at
$p<0.001$. We release code and a one-command reproduction.}
...
construct cases whose correct attribution is known in advance and check that
the method returns it. Section~\ref{sec:recovery} does this, and to our
knowledge it is the first such validation for recommender-architecture
attribution.
...
\item \textbf{C3 (primary).} A \emph{validation protocol} for source
      attribution, and evidence that SignalShap passes it. Injecting a
      controlled duplicate supplies ground truth by construction, and the
      method recovers the correct credit to numerical precision on all four
      corpora (Section~\ref{sec:recovery}). To our knowledge this is the first
      attribution method for recommender architectures validated against a
      constructed ground truth rather than assessed only by inspection.
...
The mechanism is the one Property~\ref{prop:loo} describes, and it explains why
SignalShap sees what ablation cannot. On MovieLens the collaborative signal
...
This also serves as a second validation. Measured redundancy correlates with
the attribution shift at $r = 0.491$, the relationship Lemma~\ref{lem:eps}
predicts --- the method recovers a structure we pre-registered as present
(Section~\ref{sec:setup}) rather than discovering one after the fact.
...
The strongest evidence is the validation. By injecting a known duplicate into a
real system we fixed the correct attribution by construction, and SignalShap
recovered it to numerical precision on all four corpora, spanning a $49\times$
density range. Attribution methods are usually assessed by whether their output
looks reasonable; here the answer was known in advance and the method returned
it. Under the same condition, ablation assigns zero credit to both members of
the redundant pair, which is why the two approaches recommend opposite actions
on real systems.
