...
\item \textbf{C3.} Evidence on MovieLens-1M that LOO and Shapley disagree in
      \emph{sign}, not merely in magnitude: LOO judges collaborative filtering
      harmful where Shapley ranks it second most valuable
      (\S\ref{sec:results}).
...
\paragraph{Single corpus.} All empirical results are MovieLens-1M only.
The cross-density comparison that motivates a three-corpus design
(dense movies, sparse books, medium music) is therefore \emph{not} tested
here, and no claim in this paper should be read as holding across domains.
LastFM-2K and Amazon-Book are the immediate next step; the artefact runs them
unchanged.
...
On MovieLens-1M the practical case for the method is direct: LOO scores
collaborative filtering as harmful, exact Shapley ranks it the second most
valuable source, and the two therefore recommend opposite engineering actions.
Measured redundancy tracks the gap between them. Per-user attributions differ
