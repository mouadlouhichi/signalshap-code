...
\textbf{Scope of the present results.} Results are reported on
\textbf{real MovieLens-1M} \citep{harper2015movielens} (6{,}038 users,
3{,}533 items, 575{,}276 positive interactions after the $\ge 4$-star
threshold). LastFM-2K and Amazon-Book are not yet included; their raw corpora
are pending, and the artefact runs them with the same command. Every claim
below is therefore single-corpus, and \S\ref{sec:limitations} says so
explicitly.}}
...
\paragraph{Result.} Table~\ref{tab:recovery} reports $\eta = 0$ for three
injected sources and two seeds.
...
\paragraph{Single corpus.} All empirical results are MovieLens-1M only. The
cross-density comparison is \emph{not} tested here and no claim should be read
as holding across domains. The artefact runs the remaining corpora unchanged.
We note the intervention result is a statement about the estimator rather than
about a dataset, and is therefore the part of this paper least likely to be
corpus-specific --- though that too requires confirmation.
