...
global head by only $+0.00025$ (Holm $p = 0.14$); we report the closed loop as
...
\textbf{Corpora.} Four real datasets spanning a $25\times$ density range:
MovieLens-1M \citep{harper2015movielens} ($2.70\%$), Yelp2018 ($0.160\%$),
Gowalla ($0.117\%$), and Amazon-Book ($0.106\%$), the latter three taken from
the standard LightGCN benchmark splits \citep{he2020lightgcn} so that our
preprocessing matches published practice. Two protocol differences are
disclosed rather than smoothed over: the LightGCN splits carry no timestamps,
so the leave-last-out \emph{temporal} protocol applies to MovieLens only, and
those three corpora are subsampled to 1{,}500 users to fit a 3\,GB memory
ceiling. Both are recorded in \S\ref{sec:limitations}.}}
...
\item \textbf{C5.} Segment-adaptive fusion outperforms LightGCN, SASRec,
      ...
      well-tuned global head ($d_z = 0.010$). We report the closed loop as
...
\midrule
$cf$  & $-0.00444$ & $+0.01541$ & $+0.01985$ \\
$ct$  & $+0.00083$ & $-0.00005$ & $-0.00089$ \\
$pop$ & $+0.00098$ & $+0.00632$ & $+0.00535$ \\
$rec$ & $+0.00045$ & $+0.00054$ & $+0.00009$ \\
$seq$ & $+0.01658$ & $+0.02935$ & $+0.01277$ \\
\bottomrule
\end{tabular}
\end{center}
...
$-0.0044$ --- it looks \emph{actively harmful}, and an engineer following the
standard methodology would retire it. Exact Shapley scores the same component
at $+0.0154$, the second most valuable source in the system. The disagreement
...
redundancy correlates with the LOO--Shapley gap at $0.537$, in the direction
Lemma~\ref{lem:eps} predicts.

We note $ct$ flips the other way: LOO credits it slightly positive
($+0.00083$) where Shapley returns essentially zero ($-0.00005$). Under a
...
\caption{Redundancy recovery across four real corpora spanning a $25\times$
...
\paragraph{RQ3: heterogeneity is real.} Of 30 segment-pair contrasts,
\textbf{8 are significant} at $\alpha = 0.05$ against $1.5$ expected by
chance --- more than five times the chance rate. Per-user Shapley
...
$\NDCG = 0.06391$ against $0.06366$ for globally tuned fusion --- the predicted
direction, but a margin of $+0.00025$ with Holm-corrected $p = 0.14$ and
$d_z = +0.010$. It separates decisively from every other comparator --- uniform fusion
($+0.0113$), the popularity reference ($+0.0456$), LightGCN
\citep{he2020lightgcn} ($+0.0300$, $d_z = 0.159$), and SASRec
\citep{kang2018sasrec} ($+0.0559$, $d_z = 0.286$), all at Holm-corrected
...
($d_z = 0.01$); detecting it at conventional power would need on the order of
...
$\alpha = 1.0$ --- full segment adaptation, no pooling toward global --- which
...
three LightGCN corpora are subsampled to 1{,}500 users because five dense
score matrices at full size require roughly 10\,GB against a 3\,GB ceiling.
The subsample is seeded and recorded in the artefact. It reduces statistical
power on those corpora; it does not bias the intervention, whose prediction is
fixed by construction independently of sample size.

\paragraph{The full experimental suite ran on MovieLens-1M only.} Robustness
sweeps, segment analysis, fusion, and the neural baselines are MovieLens
results. On the other three corpora we report the preconditions, attribution,
and the intervention. Completing the suite on all four is mechanical, not
conceptual.
