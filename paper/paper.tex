...
($\tau$-correlation $0.54$), as the lemma predicts. Segment-adaptive fusion
improves over globally tuned fusion in the expected direction but the margin is
not significant ($+0.00025$, Holm $p = 0.14$); we report this as an
underpowered rather than a confirmed effect. We release code, cached matrices,
...
\item \textbf{C5.} An \textbf{underpowered} result, reported as such:
      segment-adaptive fusion moves in the predicted direction but does not
      reach significance against globally tuned fusion
      (\S\ref{sec:negative}).
...
$d_z = +0.010$. It beats uniform fusion ($+0.0113$, $p < 0.001$) and the
popularity reference ($+0.0456$, $p < 0.001$) decisively, but against a
well-tuned global head it does not separate.
