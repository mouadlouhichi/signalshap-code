# SignalShap — Implementation Specification (v1.1.3, Q1-Ready, Post-Fix-Review)

> **SUPERSEDED.** This file is revision history for `v1.1.3` only. The authoritative specification is **`SignalShap_Implementation_Spec.md`**. Do not build, cite, or edit from this document.
...
> **Version 1.1.3 changelog.** One substantive patch plus two consistency scrubs. The substantive one closes a **self-defeating interaction between two v1.1.2 remedies**: the $k$-core rung of the recall fallback ladder repairs candidate recall by *raising density* — which is the exact axis the C3 density-contrast narrative is built on. Back-of-envelope, a $k=10$ core on the Amazon-Book subsample lands near $\approx 0.76\,\%$ density against LastFM-2K's $\approx 0.28\,\%$, i.e. it does not merely blunt the contrast, it **inverts the dataset ordering** and makes the "sparse books / medium music" framing false. v1.1.3 therefore (i) caps $k$ at the largest value preserving the ordering, (ii) makes the ordering a **CI-enforced invariant** rather than a prose caution, (iii) demotes $k$-core beneath the ceiling-reporting rung (it is now rung 3, was rung 2), and (iv) requires post-filter density to be recomputed and propagated to T2, §1, §4 and C3 whenever any filter is applied. Also: **density is now always reported post-filter, never quoted from the source paper**, and the §7 table gains an as-used-vs-as-published column. Scrubs: §0's internal "reviewer-hard-to-attack" vocabulary is now explicitly named in the §21 ban (Fix R12). Fix tags R10–R13, where **R13** expands Lemma 1(ii)'s derivation to pin the anchor constant at $1/|\mathcal{G}|$ and to explicitly reject two plausible-looking wrong formulas ($\tfrac12 v(\{g_1\})$ as the floor, and $\tfrac12[v(\mathcal{G})-v(\mathcal{G}\setminus\{g_1,g_2\})]$ as a closed form) that appear in a parallel draft and fail on this document's own counterexample.
...
1. **Three datasets** (ML-1M + Amazon-Book + LastFM-2K) — retire Amazon-Beauty from the analysis-doc lineage; density-contrast narrative re-derived against Amazon-Book's density. *(v1.1.3: the specific figure is no longer quoted here — density is computed as-used per §7 Fix R11, and this item is invalidated and must be regenerated if any corpus filter is applied.)*
...
15. **Density-ordering invariant** (v1.1.3) — the C3 contrast is CI-enforced against as-used statistics; the $k$-core recall remedy is capped and demoted because it can invert the ordering (§4, §7, §21).
16. **Evaluation protocol unified to full catalog** (v1.1.2) — T5 no longer compares a candidate-restricted method against full-catalog baselines (§8).
...
- **C3 — Empirical falsification of LOO under redundancy** on three benchmarks of contrasting density (dense movies, sparse books, medium music). *The parenthetical ordering is a claim about the **as-used** corpora, enforced by `tests/test_density_ordering.py`; if a corpus filter changes it, this wording is regenerated from `artefacts/dataset_stats.json` rather than retained (v1.1.3, Fix R10).*
...
1. **Raise $N_{\max}$ to 2000.** Still tractable; report the wall-clock cost honestly in T4 and re-verify the §6 compute claim holds. Cheap, no narrative change. **Preferred rung — it is the only one that changes nothing except compute.**
2. **Accept a low ceiling and report it as a finding.** A sparse-catalogue dataset where union-of-top-$N$ retrieval cannot reach $0.60$ is itself informative about the limits of the fixed-candidate game. Report attribution *conditional on* the stated ceiling, state the limitation in §5, and drop Amazon-Book from any absolute-NDCG comparison against LightGCN while retaining it for the *relative* LOO-vs-Shapley contrast — which is the claim C3 actually needs, and which is far less sensitive to the ceiling than absolute ranking quality. **Promoted above $k$-core in v1.1.3** (see the density-inversion hazard below): this rung costs a weaker absolute claim but leaves the density ordering, and therefore C3, completely intact.
3. **Tighten the corpus, not the pool — subject to the density-ordering invariant.** Apply a $k$-core filter to Amazon-Book, shrinking the catalogue rather than growing the candidate set. Standard practice, disclosable in one sentence, and it improves recall by removing items no scorer could rank anyway. **But see the hazard below: this rung is capped, CI-gated, and demoted beneath rung 2.**
...
**Density-inversion hazard on rung 3 (new in v1.1.3, Fix R10).** $k$-core repairs recall by the *same mechanism* that destroys the C3 narrative: it raises density. The two remedies are in direct tension, and the magnitude is not marginal. On the $\approx 50{,}000$-user / $\approx 90{,}000$-item / $\approx 0.05\,\%$ Amazon-Book subsample ($\approx 2.25$M interactions), order-of-magnitude estimates of the post-core density are:

| Filter | Approx. users | Approx. items | Approx. density | Status vs. LastFM-2K ($\approx 0.28\,\%$) |
|---|---|---|---|---|
| none | 50 000 | 90 000 | $\approx 0.05\,\%$ | sparsest ✓ (narrative holds) |
| $k=5$ | $\approx 27\,500$ | $\approx 25\,200$ | $\approx 0.25\,\%$ | **borderline** — essentially ties LastFM |
| $k=10$ | $\approx 15\,000$ | $\approx 10\,800$ | $\approx 0.76\,\%$ | **inverted** ✗ — Amazon-Book becomes *denser* than the "medium" dataset |
| $k=20$ | $\approx 7\,500$ | $\approx 4\,500$ | $\approx 2.33\,\%$ | **badly inverted** ✗ — approaches ML-1M |

(Illustrative retention rates; recompute against the real corpus in Week 1.) So the $k=10$ that v1.1.2 named as an option is precisely the one that breaks C3: "dense movies / sparse books / medium music" becomes factually false, and a reviewer checking T2's density column against §1's framing catches it immediately. This is worse than a blunted contrast — it is an **inverted ordering** presented as a finding.

**Therefore, three binding rules on rung 3:**

- **(a) $k$ is capped by the ordering, not chosen for convenience.** Select the largest $k\in\{3,5,8,10\}$ whose *measured* post-filter density keeps the strict ordering $\rho_{\text{Amazon-Book}} < \rho_{\text{LastFM-2K}} < \rho_{\text{ML-1M}}$ with a margin of at least $1.5\times$ between adjacent datasets. If no $k$ in that set both clears the recall gate and preserves the ordering, **rung 3 is unavailable** — fall through to rung 4.
- **(b) The ordering is a CI invariant, not a caution.** `tests/test_density_ordering.py` asserts the strict ordering and the $1.5\times$ margins against the *as-used, post-filter* statistics in `artefacts/dataset_stats.json`. Any filter that inverts or compresses the ordering fails the build. Prose warnings do not survive a Week-6 deadline; a red test does.
- **(c) Every downstream density claim is regenerated, not edited by hand.** If any filter is applied, recompute density and re-propagate to T2, the §1 framing sentence, §4's motivating "$\approx 0.05\,\%$" figure, C3's wording, and the §0 selection table — all from the same artefact JSON. Item 1 of §1 ("density-contrast narrative re-derived against Amazon-Book's ~0.05 % density") is invalidated the moment a core filter runs.

**The chosen rung is recorded in the artefact** with its date, the recall numbers that triggered it, the filter parameters, and the pre- and post-filter density of every dataset.
...
- **Lemma 1 ($\varepsilon$-redundancy) — new in v1.1.2, Fix R4.** Exact redundancy is a measure-zero condition that will never hold on real data, so Property 2 can motivate E2 but can never be *instantiated* by it. Lemma 1 supplies the approximate version that E2 actually tests, and it necessarily has **two separate parts under two separate assumptions** — the counterexample above is exactly the reason the second part cannot be derived from redundancy alone:
  ...
  - **(ii) Shapley floor.** If **additionally** $v$ is monotone and $v(\{g_1\})\ge c$, then $\varphi_{g_1}\ \ge\ c/|\mathcal{G}| - O(\varepsilon)$. Requires the permutation-averaging argument, and requires monotonicity — it does **not** follow from $\varepsilon$-redundancy.
    ...
    *Derivation of the constant (expanded in v1.1.3, Fix R13).* Under exact redundancy $g_1$'s marginal contribution vanishes for every coalition already containing $g_2$, so $\varphi_{g_1}$ collapses to a weighted sum over coalitions $T\subseteq\mathcal{G}\setminus\{g_1,g_2\}$: $\varphi_{g_1}=\sum_T w(|T|)\,[v(T\cup\{g_1\})-v(T)]$. **Two distinct quantities must not be conflated here.** The weights $w(|T|)$ over that index set sum to exactly $1/2$ — this is $\Pr[g_1 \text{ precedes } g_2]$ in a uniformly random ordering, and it holds for every $n$. But the term that actually produces the $v(\{g_1\})$ anchor is $T=\varnothing$, whose weight is $w(0)=\tfrac{0!\,(n-1)!}{n!}=1/n$. Under monotonicity every remaining term is $\ge 0$, so the anchor term alone gives $\varphi_{g_1}\ge v(\{g_1\})/n = v(\{g_1\})/|\mathcal{G}|$.

    **The constant is $1/|\mathcal{G}|$, never $1/2$.** Substituting the total mass $1/2$ for the anchor weight $1/n$ overstates the bound by $n/2$ — a factor of $2.5$ for this paper's five-player game — and is not recoverable by any choice of $\varepsilon$. Equally, **no pairwise closed form of the shape $\tfrac12[v(\mathcal{G})-v(\mathcal{G}\setminus\{g_1,g_2\})]$ is valid**: it discards every sub-grand coalition and returns $-2.25$ on the §5 counterexample against a true $\varphi_{g_1}=-0.4167$. Both errors are pinned as rejection tests in `tests/test_property2_counterexample.py` (`test_pairwise_closed_form_is_wrong`, `test_shapley_floor_constant_is_one_over_n_not_one_half`) so neither can re-enter §5.
...
| Dataset | Domain | Users | Items | Density (as published) | Filters applied | Density (as used — authoritative) | Split | License / Terms of Use |
|---|---|---|---|---|---|---|---|---|
| MovieLens-1M | Movies | 6 040 | 3 706 | ≈ 4.5 % | none | *recompute* | Leave-last-out temporal | GroupLens research-use license (research and educational use, redistribution with attribution) |
| Amazon-Book (2018) | Books | ≈ 50 000 subsampled from the 2018 raw corpus | ≈ 90 000 | ≈ 0.05 % | subsample (seeded); $k$-core **only** if rung 3 is triggered and passes the ordering invariant | *recompute* | Leave-last-out temporal | Amazon Reviews Dataset (Ni et al., 2019), academic use permitted per the dataset host's conditions |
| LastFM-2K | Music | 1 892 | 17 632 | ≈ 0.28 % | none | *recompute* | Leave-last-out temporal | HetRec 2011 / GroupLens redistribution, non-commercial research use |

**Density is reported as-used, never as-published (new in v1.1.3, Fix R11).** The "as-published" column above is provenance only. Every density figure that appears in the paper — T2, the §1 framing sentence, §4's motivating figure, C3's wording — is **computed from the actual post-split, post-filter matrices** by `src/signalshap/data/stats.py` and written to `artefacts/dataset_stats.json`; the manuscript reads from that JSON. This rule exists because the subsample alone (50 k of the raw Amazon-Book user base) already shifts density away from the published figure before any $k$-core is considered, so quoting $\approx 0.05\,\%$ from the source paper is unsafe even on the current plan. The ordering invariant of §4 is asserted against these computed values.
...
│   ├── test_density_ordering.py           # C3 density-contrast invariant, as-used stats (v1.1.3)
...
| **$k$-core recall remedy inverts the density ordering and falsifies C3** | (mitigated) | **Would have been high** — an inverted ordering shipped as a finding | $k$ capped by the ordering invariant, rung demoted beneath ceiling-reporting, CI test `test_density_ordering.py`, all density figures regenerated from artefact JSON (Fix R10/R11). |
...
- **Week 1 — Scaffolding, pre-registration, and the two gates.** Repo, CI, Docker (CPU + GPU), dataset download scripts, temporal split, deterministic tie-breaking. The proportional-growth candidate builder from §4 including the **truncation rule**, with three unit tests: identical $C_u$ across all $2^5$ coalition contexts, identical $C_u$ across the five seeds, and $|C_u|\le N_{\max}^{(d)}$ for every user. **Pre-register $N_{\max}^{(d)}$ per dataset and freeze it in `configs/`.** Run **E0-a candidate recall on Amazon-Book first** — highest-risk fact, ~1 day, do it before writing anything downstream. Serialise the frozen permutation $\pi_u$ for $v_0$. **Build `artefacts/dataset_stats.json` (as-used users/items/interactions/density per dataset) and turn on `tests/test_density_ordering.py` before any filter decision is taken** — if rung 3 is ever reached, the invariant must already be live. Synthetic 32-coalition smoke test checking Property 1 to floating-point tolerance, **plus `tests/test_property2_counterexample.py`**, which pins the three-player counterexample of §5 so the monotonicity hypothesis is enforced by CI rather than by prose.
...
- **Candidate recall is a submission gate — with a real remediation path (revised in v1.1.2).** The gate is now evaluated at the **pre-registered per-dataset $N_{\max}^{(d)}$** of §4. If mean candidate recall falls below $0.60$, follow the **four-rung fallback ladder in §4** — raise $N_{\max}$ → report the ceiling as a finding and restrict Amazon-Book to the relative LOO-vs-Shapley claim → $k$-core the corpus *subject to the density-ordering invariant* → substitute Gowalla — in that order (**$k$-core demoted beneath the ceiling-reporting rung in v1.1.3**), recording the chosen rung and its compute cost in the artefact. Do *not* re-tune the growth schedule: it redistributes budget between sources and cannot create recall the base scorers' top-lists do not contain. Note that rung 1 is bounded by the §6 compute claim — if the pool needed to clear the gate breaks the laptop-CPU story for that dataset, escalate rather than keep growing. Any change to $N_{\max}^{(d)}$ after Week 1 must be recorded in the artefact with its date and reason. Restricting the evaluated user population to those with $\text{test}_u\in C_u$ is **forbidden**: it biases the population differentially by density and confounds C3. Enforced in CI; E0-a exits non-zero on breach.
- **The density ordering is a CI invariant, and C3 lives or dies by it (new in v1.1.3).** $\rho_{\text{Amazon-Book}} < \rho_{\text{LastFM-2K}} < \rho_{\text{ML-1M}}$ with $\ge 1.5\times$ margins, asserted by `tests/test_density_ordering.py` against *as-used* statistics. No corpus filter may be applied without re-running it. If the ordering cannot be preserved, the filter is reverted or C3 and the §1 framing are rewritten — the one forbidden outcome is shipping an inverted ordering under the old "dense movies / sparse books / medium music" wording.
- **Density is computed, never quoted.** Every density figure in the manuscript reads from `artefacts/dataset_stats.json`, built from the post-split, post-filter matrices. Published figures appear only in T2's provenance column. This applies even absent a $k$-core, since the 50 k Amazon-Book subsample already moves density off its published value.
...
- **§0's internal vocabulary never reaches the manuscript (tightened in v1.1.3, Fix R12).** "Reviewer attack surface," "acceptance risk," "reviewer-hard-to-attack" (§0 Verdict), "minimal attack surface" (§0 table) and the whole four-blueprint comparison table are planning artefacts. The existing ban on "review-proof" framing extends to all of it, including the cover letter.

---

**End of implementation specification, v1.1.3.**
