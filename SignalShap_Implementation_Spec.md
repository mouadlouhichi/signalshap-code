...
| 67 | `run_protocol_sensitivity.py` accepted `--budget-gb` and never used it: no `size_corpus` call, so a Gowalla run loaded the full 52,985 x 121,866 corpus instead of the sized 8,865 x 82,134 and was killed. The flag looked honoured because argparse parsed it | Script now sizes before loading and records `corpus_shape`, refusing to report a corpus whose shape does not match the manuscript via the existing `check_paper_shape` guard. Gowalla subsequently ran at the correct shape. Test asserts both `size_corpus` and `check_paper_shape` appear | §3.2 |
| 68 | Same script started from an empty dict rather than resuming, so `--corpora gowalla_ts` silently deleted the ml_1m and amazon blocks already on disk. This is provenance entry 5 reintroduced in a new script | Loads the existing artefact on start and merges. Lost blocks recovered from git. Test pins the resume behaviour | §3.2 |
| 69 | The real baseline contamination in the end-to-end retirement losses is 6.7e-4, four times the 1.7e-4 bound estimated from the synthetic pilot. Table 7 quoted centred losses to five decimals | Table 7 regenerated from raw utility on all three corpora; every entry moved. Conclusions unchanged: cheapest source still `cf`, tau_LOO 1.00, tau_Shapley 0.20. The paper now states the measured 6.7e-4 rather than the pilot estimate | §13 |
| 70 | `check_paper_numbers.py` compared the prose against `results_ml_1m.json`, which predates the symmetric candidate rule, and flagged a phantom drift in Shapley(seq) after Table 7 was regenerated | Checker now prefers `final_seed_ci.json`, the ten-seed artefact the tables actually quote | §20 |
...
| 71 | Gowalla's candidate-recall shortfall was attributed entirely to catalogue sparsity and pool affordability, and the manuscript declined to say the 0.60 gate was unreachable only because that would have meant extrapolating the log recall fit. The repeat audit shows the real cause: **49.90% of Gowalla's evaluated users have a test venue already in their training history**, which `mask_seen` sets to -inf in every source, so the item enters no candidate set, NDCG_u = 0 and b_u = 0, and v_u(S) = 0 identically for all 32 coalitions. Recall is hard-bounded by rho = 0.501, so the gate is unreachable at ANY N_max, and 0.450 is 0.898 of the attainable ceiling rather than a poor score | Table `tab:repeats` added with the measured counts for all three corpora. The dilution identity v(S) = rho * v_live(S) is stated as Eq. (14) and verified to 1.4e-17 over all 32 coalitions; by linearity phi_g = rho * phi_g_live, so every Gowalla ordering, sign, Kendall tau and ratio-to-v(G) is EXACTLY invariant while every absolute magnitude is halved (v(G) 0.01705 diluted vs 0.03403 live). Diluted values are reported, since U_eval is the declared population and rescaling after seeing rho would be selection, but absolute Gowalla levels are now flagged as non-comparable across corpora. `tests/test_masked_user_dilution.py` (6 tests) pins the identity; `check_paper_numbers.py` binds every cell of the new table plus rho to the artefact, and its old guard demanding a RETRACTION of the impossibility claim now demands the measured ceiling as EVIDENCE instead | §2.2, §2.5 |
| 72 | The manuscript warned that the `rec` argmax cluster partition "is not invariant to an arbitrary sign reversal of an SVD component", so reproduction supposedly required the exact cached decomposition. Reviewer item 11 asked for a deterministic sign convention, and we scoped it as a fix that would invalidate cached runs | Implemented `canonical_svd_sign` (orient each component so its largest-magnitude loading is positive) and applied it before the argmax; `rec` is now provably bit-identical across solver seeds, tested. **The caveat was wrong, though, and the fix is a no-op:** `sklearn.decomposition.TruncatedSVD` already calls `svd_flip(U, VT, u_based_decision=False)` on both its code paths, which is the identical convention, so component signs were never the hazard and NO cached run is invalidated. The guard is retained because that is an sklearn implementation detail, not an API promise. The genuine residual hazard is different in kind: rotation inside a TIED singular subspace, which no sign convention can repair. `tests/test_svd_sign_determinism.py` exhibits it on a matrix with a repeated singular value, and the paper's caveat is restated against degeneracy | §4 |
| 73 | The candidate pool is coalition-independent but still built BY the five players, so it is enriched for the items they rank highly and nothing measured whether the attribution depends on that | New `scripts/run_pool_sensitivity.py` rebuilds the whole game on three source-blind pools: global popularity, keyed-hash random, and random-with-oracle-target. The oracle rule exists because the plain random pool collapses recall to N/|I|, which flattens the game through the Eq. (14) dilution and would confound "attribution is pool-robust" with "the game is empty". Six tests, the load-bearing one permuting score MAGNITUDES while preserving eligibility and asserting the neutral pools are unmoved AND the union pool IS moved, so the contrast cannot be vacuous | §2.1 |
| 74 | The frozen-versus-refreshed protocol contrast was seed 42 only, so it could not separate a protocol effect from run-to-run noise | `block_temporal_seeds` runs the paired contrast over the same ten seeds as the main results, with percentile-bootstrap intervals (not t_9, which overran the admissible range for bounded statistics) and per-source sign counts. **Found a bug in my own aggregator while testing it:** `n_unit` counted `kendall_tau == 1.0` exactly, but scipy returns 0.9999999999999999 for a perfectly preserved ordering, so it would have reported "0 of 10 seeds preserve the ordering" when all ten did -- a false negative that would have UNDERSTATED the method's stability. Fixed with a tolerance and pinned by `test_unit_tau_is_counted_with_tolerance` | §11 |
| 75 | The blocked global-time replication reported attribution vectors only, while the paper's central operational claim is about RETIREMENT, so the claim that most needed a causal check did not get one | `run_global_timeblock.py --retirement` drives the same `retirement_simulation` on the blocked split through a duck-typed `_BlockedExperiment`. Constructing a real `Experiment` would have reloaded the corpus and rebuilt leave-last-out folds, silently discarding the global cutoff. `test_the_duck_typed_experiment_supplies_every_attribute_read` greps the estimands module for `exp.<attr>` and asserts `__slots__` covers all of them, so a future field cannot read stale state | §12 |
| 76 | Six separate multi-hour commands, run by hand, with a `run_study.py` invocation that must not batch corpora (it refuses differing user caps) | `scripts/run_round8_remaining.sh`: independent resumable stages, no `set -e` (a failed stage must not discard hours of successful work), no `timeout` (absent on macOS, and this runs on an M4), failures collected and reported with a non-zero exit. Guarded by ten tests, including one that reads each target script's parser and asserts every `--flag` passed is actually defined -- mutation-tested by mistyping `--retirement`, which the suite catches before the run starts rather than after | §20 |

| 77 | `notebooks/SignalShap_M4_FullStudy.ipynb` is described in its README as the notebook to "start here" for the full study, but it runs `yelp2018`, `gowalla` and `amazon_book_lgcn` -- the untimestamped LightGCN splits, NOT the three corpora the paper reports (`ml_1m`, `amazon_video_games`, `gowalla_ts`). Three of five players need real timestamps, which those splits discard. Running it to produce round-8 artefacts would have written plausible results for the wrong datasets | New generated notebook `SignalShap_Round8_Runs.ipynb` covering exactly the open items on exactly the paper's corpora, with the mismatch called out in its first cell and in `notebooks/README.md`. `test_only_the_papers_corpora_are_run` fails if any LightGCN corpus name appears in it | §10 |
| 78 | A hand-maintained `.ipynb` drifts from the scripts it calls, and its diff is unreadable, so the drift is invisible in review | The notebook is GENERATED by `scripts/make_round8_notebook.py` with content-hashed cell ids (stable across regenerations), and `test_notebook_matches_its_generator` builds it in memory and compares against the committed file. **My first version of that test regenerated the file before comparing, so it could never fail;** caught by mutation-testing a hand edit, which passed. Fixed to import the generator and never touch disk, then re-verified: the hand edit now fails the suite | §10 |
| 79 | Notebook helper code shipped untested. Executing the non-run cells against the real artefacts found two live bugs: `show_pools` called `read_artefact` on a not-yet-written artefact and raised `FileNotFoundError` in the normal pre-run state, and the summary cell printed "ALL STAGES OK" for an empty results table, i.e. when nothing had run at all | Both fixed and pinned by `test_reporting_cells_tolerate_a_missing_artefact` and `test_empty_summary_does_not_claim_success`. The `run()` helper was separately exercised on a passing and a deliberately failing command to confirm it streams output, records the exit code and continues | §10 |

| 80 | The round-8 notebook set `REPO = Path.home()/"signalshap-code"` guarded only by `assert REPO.exists()`. The user had a STALE EMPTY directory at exactly that path while the real clone lived under `Desktop/personal/phd/next-paper/`. The assert passed, the notebook chdir'd into the empty directory, and all nine stages failed instantly with "can't open file scripts/run_study.py". Nothing ran (total 0 min, so no compute was wasted) but nothing warned either, and the summary correctly reported 9 FAILED without saying WHY | `_find_repo()` discovers the repo by CONTENTS (`scripts/run_study.py`, `src/signalshap/config.py`, `configs/frozen.yaml` must all be present), walking up from the cwd first so it is correct wherever the clone lives, then trying conventional locations. If nothing matches it raises with an explicit override snippet instead of proceeding. A directory existing is not evidence that it is the RIGHT directory. Also added a warning when the kernel interpreter differs from the repo's `.venv`, since stages shell out with `sys.executable` and would otherwise silently get a different environment than the cells. Regression-tested by executing the real setup cell with `HOME` pointed at a decoy empty directory, and mutation-tested by restoring the hardcoded path, which fails three tests | §10 |

| 81 | Regenerating the ML-1M and Amazon diagnostics under the source-symmetric candidate rule moved four numbers the manuscript quoted from the legacy run, and `check_paper_numbers.py` caught all four at once: fusion NDCG, global-head NDCG, the C4 heterogeneity count, and the ML-1M raw monotonicity count | All four corrected from the artefacts. **The consequential one is a SIGN FLIP:** the attribution-derived fusion head is now 0.00009 BELOW the matched global head (Holm p=0.23), where the legacy run had it 0.00003 above. Appendix A's negative result is strengthened, not weakened, and the manuscript now states that a quantity whose sign depends on the truncation key is not evidence of an advantage in either direction. Raw monotonicity moved 23->26 (ML-1M) and 12->14 (Amazon) while the MATERIAL counts held at 7 and 4, which is a third and fourth data point for the reproducibility argument the paper already makes; the prose now cites three runs rather than two. Segment heterogeneity 8/30->7/30 (ML-1M) and 5/30->7/30 (Amazon) | §14 |
| 82 | `run_study.py --datasets gowalla_ts` was OOM-killed after 116 minutes on a 48 GB machine: five dense 8,865 x 82,134 float32 score matrices are 14.6 GB before the game allocates anything, and the full E0-E8 suite holds several derived structures on top | Not worked around, and not hidden. Gowalla's sweeps, segments and preconditions remain under the legacy candidate rule, and Table `tab:provenance` now carries an explicit footnote saying which corpora the symmetric-rule regeneration covers and why Gowalla is absent. This does NOT touch Gowalla's primary numbers: attribution and retirement come from `final_seed_ci.json`, which is source-symmetric on all three corpora. A chunked scorer would be the fix if the diagnostics ever need to be confirmatory | §2.2 |
| 83 | The blocked-split retirement run (review item 3) was the one result with a real chance of contradicting the paper's central claim, since the per-source attributions demonstrably do NOT replicate under a global cutoff (tau 0.60 for Shapley, -0.40 for LOO on the ALLOCATION vectors) | It replicated. Against observed removal cost, tau_LOO = 0.80 and tau_Shapley = 0.60, an advantage of +0.20 in the same direction as the main result, and LOO identifies the truly cheapest source (`pop`) while the allocation names `rec` and is wrong. The manuscript now separates the two explicitly: per-source attributions are specific to per-user chronological evaluation, while the allocation-versus-removal DISAGREEMENT survives a globally causal split. That separation is a stronger and narrower claim than the one the paper made before the run | §12 |
| 84 | The neutral-pool oracle rule returned candidate recall of exactly 1.000 / 0.972 / 0.501 on the three corpora, which initially looked like a bug: the rule inserts the test item by construction, so recall "should" be 1.000 everywhere | Not a bug, and it is a free confirmation of Eq. (14). The oracle can only insert a target that is ELIGIBLE, and `mask_seen` has already removed every test item the user consumed in training. So oracle recall equals rho, the retrievability ceiling, and it reproduced the repeat-audit values (1.000 / 0.972 / 0.501) to three decimals from a completely independent code path. Reported in the manuscript as such | §2.2 |

| 85 | The copy from `artefacts/figures/F<N>_*.png` into `paper/figures/Fig<N>.png` was MANUAL and undocumented. After the symmetric-rule regeneration the committed figures were eleven days stale, so Figure 6 plotted the LEGACY fusion comparison (globally-tuned 0.063, SignalShap-Fuse 0.063) while the prose beside it quoted the regenerated values -- including the one that had changed SIGN. A reviewer comparing figure to text would have found them contradicting each other, and nothing in the build warned | `scripts/make_assets.py` now publishes into `paper/figures/` as its final step, with `--no-publish` for diagnostics and a hard guard that `--include-failed` assets are never published. `tests/test_figures_are_current.py` checks every included figure exists, has no orphans, and is not older than its source in `artefacts/figures/`; mtime rather than hash, because matplotlib output is not byte-reproducible across versions. Mutation-tested by backdating Fig6.png, which reproduces the shipped bug and fails | §13 |
| 86 | Five of seven figures and all seven tables ordered corpora ALPHABETICALLY (amazon, gowalla, ml_1m), because `results` is built by globbing `results_*.json`. Every table in the manuscript reports dense to sparse with MovieLens first, so a regeneration silently reordered panels away from the text and buried the reference corpus on the right. F3 had already hit this and worked around it locally, which is why the inconsistency survived: one figure looked right | `corpus_order()` promoted to a shared helper in `plots/assets.py` and applied to all five figure builders, all seven table builders, and F3's local workaround. The ordering is now one decision rather than six independent ones | §13 |
